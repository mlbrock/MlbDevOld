...
/*
	...
	File Description	:	Adds a new node before a specified reference node
...
/*	***********************************************************************	*/
DLLS_NODE *DLLS_AddPrevious(DLLS_NODE *ref_node_ptr, DLLS_NODE *new_node_ptr)
{
	if (ref_node_ptr->dlls_ptr->first_ptr == ref_node_ptr)
		DLLS_AddFirst(ref_node_ptr->dlls_ptr, new_node_ptr);
	else {
		new_node_ptr->dlls_ptr           = ref_node_ptr->dlls_ptr;
		new_node_ptr->prev_ptr           = ref_node_ptr->prev_ptr;
		new_node_ptr->next_ptr           = ref_node_ptr;
		ref_node_ptr->prev_ptr->next_ptr = new_node_ptr;
		ref_node_ptr->prev_ptr           = new_node_ptr;
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_AddPrevious()'\n");
	fprintf(stderr, "---- ------- --- -------- --------------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		if ((return_code = DLLS_TEST_AllocateNode(&test_data, &tmp_node,
			...
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("Before 'DLLS_AddPrevious()'\n");
			DLLS_TEST_EmitDLLS(&test_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			DLLS_AddPrevious(test_data.dlls_data.first_ptr->next_ptr->next_ptr,
				&tmp_node->node_data);
			printf("\n");
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_AddPrevious()' %s, instance %lu\n",
