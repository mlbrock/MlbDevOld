...
/*
	...
	File Description	:	Deletes the node after a specified reference node
...
/*	***********************************************************************	*/
void DLLS_DeleteNext(DLLS_NODE *ref_node_ptr)
{
	DLLS_NODE *node_ptr;

	if ((node_ptr = ref_node_ptr->next_ptr) != NULL) {
		DLLS_RemoveNext(ref_node_ptr);
		if (ref_node_ptr->dlls_ptr->free_func != NULL)
			(*ref_node_ptr->dlls_ptr->free_func)(node_ptr->data_ptr);
	}
}
/*	***********************************************************************	*/
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_DeleteNext()'\n");
	fprintf(stderr, "---- ------- --- -------- -------------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		for (count_1 = 0; count_1 < (node_count - 1); count_1++) {
			...
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_DeleteNext()', number %u of %u, instance %lu\n",
				count_1 + 1, node_count,
				((TEST_NODE *) tmp_node->data_ptr)->instance);
			DLLS_DeleteNext(test_data.dlls_data.first_ptr);
			DLLS_TEST_EmitDLLS(&test_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
