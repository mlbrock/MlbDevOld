...
/*
	...
	File Description	:	Deletes the node before a specified reference node
...
/*	***********************************************************************	*/
void DLLS_DeletePrevious(DLLS_NODE *ref_node_ptr)
{
	DLLS_NODE *node_ptr;

	if ((node_ptr = ref_node_ptr->prev_ptr) != NULL) {
		DLLS_RemovePrevious(ref_node_ptr);
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_DeletePrevious()'\n");
	fprintf(stderr, "---- ------- --- -------- -----------------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		STR_EMIT_CharLine('*', 78, NULL, NULL);
		printf("Before 'DLLS_DeletePrevious()'\n");
		...
		for (count_1 = 0; count_1 < (node_count - 1); count_1++) {
			...
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_DeletePrevious()', number %u of %u, instance %lu\n",
				count_1 + 1, node_count,
				((TEST_NODE *) tmp_node->data_ptr)->instance);
			DLLS_DeletePrevious(test_data.dlls_data.last_ptr);
			DLLS_TEST_EmitDLLS(&test_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
