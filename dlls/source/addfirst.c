...
/*
	...
	File Description	:	Adds a new node to the beginning of a doubly-linked
								list.
...
/*	***********************************************************************	*/
DLLS_NODE *DLLS_AddFirst(DLLS *dlls_ptr, DLLS_NODE *new_node_ptr)
{
	new_node_ptr->prev_ptr = NULL;
	new_node_ptr->dlls_ptr = dlls_ptr;

	if (dlls_ptr->first_ptr != NULL) {
		new_node_ptr->next_ptr        = dlls_ptr->first_ptr;
		dlls_ptr->first_ptr->prev_ptr = new_node_ptr;
	}
	else {
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_AddFirst()'\n");
	fprintf(stderr, "---- ------- --- -------- -----------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		if ((return_code = DLLS_TEST_AllocateNode(&test_data, &tmp_node,
			...
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("Before 'DLLS_AddFirst()'\n");
			DLLS_TEST_EmitDLLS(&test_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			DLLS_AddFirst(&test_data.dlls_data, &tmp_node->node_data);
			printf("\n");
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_AddFirst()', DLLS node 0x%8p\n",
				&tmp_node->node_data);
			DLLS_TEST_EmitDLLS(&test_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
