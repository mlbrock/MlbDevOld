...
/*
	...
	File Description	:	Removes the first node in a doubly-linked list.
...
/*	***********************************************************************	*/
void DLLS_RemoveFirst(DLLS *dlls_ptr)
{
	if (dlls_ptr->first_ptr != NULL) {
		if (dlls_ptr->first_ptr == dlls_ptr->last_ptr) {
			...
			dlls_ptr->first_ptr           = dlls_ptr->first_ptr->next_ptr;
			dlls_ptr->first_ptr->prev_ptr = NULL;
		}
		dlls_ptr->node_count--;
	}
}
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_RemoveFirst()'\n");
	fprintf(stderr, "---- ------- --- -------- --------------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		STR_EMIT_CharLine('*', 78, NULL, NULL);
		printf("Before 'DLLS_RemoveFirst()'\n");
		...
		for (count_1 = 0; count_1 < node_count; count_1++) {
			...
			DLLS_RemoveFirst(&test_data.dlls_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_RemoveFirst()', number %u of %u, instance %lu\n",
