...
/*
	...
	File Description	:	Deletes the last node in a doubly-linked list.
...
/*	***********************************************************************	*/
void DLLS_DeleteLast(DLLS *dlls_ptr)
{
	DLLS_NODE *node_ptr;

	if ((node_ptr = dlls_ptr->last_ptr) != NULL) {
		DLLS_RemoveLast(dlls_ptr);
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_DeleteLast()'\n");
	fprintf(stderr, "---- ------- --- -------- -------------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		for (count_1 = 0; count_1 < node_count; count_1++) {
			...
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_DeleteLast()', number %u of %u, instance %lu\n",
				count_1 + 1, node_count,
				((TEST_NODE *) tmp_node->data_ptr)->instance);
			DLLS_DeleteLast(&test_data.dlls_data);
			DLLS_TEST_EmitDLLS(&test_data);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
