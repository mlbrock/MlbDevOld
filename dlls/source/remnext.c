...
/*
	...
	File Description	:	Removes the node after a specified reference node
...
/*	***********************************************************************	*/
void DLLS_RemoveNext(DLLS_NODE *ref_node_ptr)
{
	if (ref_node_ptr->next_ptr != NULL) {
		if (ref_node_ptr->next_ptr == ref_node_ptr->dlls_ptr->last_ptr)
			DLLS_RemoveLast(ref_node_ptr->dlls_ptr);
		else {
			ref_node_ptr->next_ptr           = ref_node_ptr->next_ptr->next_ptr;
			ref_node_ptr->next_ptr->prev_ptr = ref_node_ptr;
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Test harness for function 'DLLS_RemoveNext()'\n");
	fprintf(stderr, "---- ------- --- -------- -------------------\n\n");
	...
	if ((return_code = DLLS_TEST_AddNodes(&test_data, test_count, DLLS_FALSE,
		...
		STR_EMIT_CharLine('*', 78, NULL, NULL);
		printf("Before 'DLLS_RemoveNext()'\n");
		DLLS_TEST_EmitDLLS(&test_data);
		STR_EMIT_CharLine('*', 78, NULL, NULL);
		node_count = test_data.dlls_data.node_count;
		for (count_1 = 0; count_1 < (node_count - 1); count_1++) {
			printf("\n");
			tmp_node = test_data.dlls_data.first_ptr->next_ptr;
			DLLS_RemoveNext(test_data.dlls_data.first_ptr);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			STR_EMIT_CharLine('*', 78, NULL, NULL);
			printf("After 'DLLS_RemoveNext()', number %u of %u, instance %lu\n",
