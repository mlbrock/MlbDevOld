...
/* BOH
   ...
   NAME        :  strli_search_delta

   SYNOPSIS    :  return_code = strli_search_delta(in_count_1, in_list_1,
   ...
   DESCRIPTION :  Determines the delta between the elements in two arrays
						...
						using ``STR_stricmp`` as a comparison functions.
   ...
   OUTPUT INDEX:  strli_search_delta
						String List Search Delta Functions:strli_search_delta
						STRFUNCS:String List Search Delta Functions:strli_search_delta
						String Functions:See STRFUNCS

   PUBLISH XREF:  strli_search_delta

   PUBLISH NAME:	strli_search_delta
...
/* *********************************************************************** */
#ifndef NARGS
int strli_search_delta(unsigned int in_count_1, char **in_list_1,
...
#else
int strli_search_delta(in_count_1, in_list_1, in_count_2, in_list_2,
...
#endif /* #ifndef NARGS */
{
	...
	fprintf(stderr, "Test of function 'strli_search_delta()'\n");
	fprintf(stderr, "---- -- -------- ----------------------\n\n");
	...
	else if ((return_code = strli_search_delta(file_count_1, file_list_1,
...
#endif /* #ifndef NARGS */
{
	...
	*file_list  = NULL;
	...
	if ((file_ptr = fopen(file_name, "r")) == NULL) {
		...
		fclose(file_ptr);
		if (!return_code)
			strli_sort(*file_count, *file_list);
	}
