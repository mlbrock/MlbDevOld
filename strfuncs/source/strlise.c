...
/* BOH
   ...
   SYNOPSIS    :  return_code = strli_search(string_count, string_list,
   ...
   OUTPUT INDEX:  strli_search
						String List Functions:strli_search
						STRFUNCS:String List Functions:strli_search
						String Functions:See STRFUNCS

   PUBLISH XREF:  strli_search

   PUBLISH NAME:	strli_search
...
/* *********************************************************************** */
#ifndef NARGS
int strli_search(unsigned int *string_count, char ***string_list,
	const char *key_string, unsigned int *found_index)
#else
int strli_search(string_count, string_list, key_string, found_index)
unsigned int   *string_count;
char         ***string_list;
const char     *key_string;
unsigned int   *found_index;
...
#else
	return(STR_LIST_search(string_count, ((void ***) string_list),
		((void *) key_string), strlen(key_string) + 1, found_index, STR_stricmp,
		memlins));
#endif /* #ifndef NARGS */
}
/* *********************************************************************** */
...
int main()
{
	...
	fprintf(stderr, "Test of function 'strli_search()'\n");
	fprintf(stderr, "---- -- -------- ----------------\n\n");
	...
	while ((!feof(stdin)) && (!ferror(stdin))) {
		*buffer = '\0';
		...
		if (!stricmp(buffer, "LIST")) {
			...
			(!stricmp(buffer, "EXIT")))
			break;
		else if ((return_code = strli_search(&test_count, &test_list,
	...
	if (!return_code) {
		if (test_count != saved_count) {
			...
			strli_sort(saved_count, saved_list);
			fprintf(stderr, "\nChecking . . .");
			for (count_1 = 0; count_1 < saved_count; count_1++) {
				if ((return_code = strli_find(test_count, test_list,
