...
/*
	...
	File Description	:	Basic array of pointers non-search delta logic.
...
/* BOH
   ...
   SYNOPSIS    :  return_code = MEM_LIST_delta(control_ptr, in_count_1,
   ...
   DESCRIPTION :  Determines the delta between the elements in two arrays
						of pointers using a user-supplied comparison function.
   ...
   OUTPUT INDEX:  MEM_LIST_delta
						...
						STRFUNCS:Memory List Delta Functions:MEM_LIST_delta
						String Functions:See STRFUNCS

   PUBLISH XREF:  MEM_LIST_delta

   PUBLISH NAME:	MEM_LIST_delta
...
/* *********************************************************************** */
#ifndef NARGS
int MEM_LIST_delta(void *control_ptr, unsigned int in_count_1, void **in_list_1,
...
#else
int MEM_LIST_delta(control_ptr, in_count_1, in_list_1, in_count_2, in_list_2,
		...
		if ((!in_count_1) || (!in_count_2)) {
			...
			for (count_1 = 0; count_1 < max_count; count_1++) {
				...
				if ((!uniq_flag_1) && (!uniq_flag_2)) {
					if ((*compare_function)(control_ptr, in_list_1[count_1],
						in_list_2[count_1], data_length)) {
...
#endif /* #ifndef NARGS */
{
	...
	fprintf(stderr, "Test of function 'MEM_LIST_delta()'\n");
	fprintf(stderr, "---- -- -------- ------------------\n\n");
...
#ifndef NARGS
	else if ((return_code = MEM_LIST_delta(NULL, file_count_1, file_list_1,
