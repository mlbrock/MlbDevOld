...
/*
	...
	File Description	:	Performs a case-insensitive binary search on an array
								of pointers to strings to insert a string.
...
/* BOH
   ...
   SYNOPSIS    :  return_code = strlni_search(string_count, string_list,
   ...
   RETURNS     :  Returns from this function are as follow:
						...
						(.) ''STRFUNCS_ITEM_FOUND'' if ``key_string`` is found
   ...
   OUTPUT INDEX:  strlni_search
						String List Functions:strlni_search
						STRFUNCS:String List Functions:strlni_search
						String Functions:See STRFUNCS

   PUBLISH XREF:  strlni_search

   PUBLISH NAME:	strlni_search
...
/* *********************************************************************** */
#ifndef NARGS
int strlni_search(unsigned int *string_count, char ***string_list,
	const char *key_string, size_t string_length, unsigned int *found_index)
#else
int strlni_search(string_count, string_list, key_string, string_length,
	found_index)
unsigned int   *string_count;
char         ***string_list;
const char     *key_string;
...
int main()
{
	...
	fprintf(stderr, "Test of function 'strlni_search()'\n");
	fprintf(stderr, "---- -- -------- -----------------\n\n");
	...
	while ((!feof(stdin)) && (!ferror(stdin))) {
		*buffer = '\0';
		...
		if (!STRFUNCS_stricmp(buffer, "LIST")) {
			...
			break;
		else if ((return_code = strlni_search(&test_count, &test_list,
	...
	if (!return_code) {
		if (test_count != saved_count) {
			...
			strlni_sort(saved_count, saved_list, max_length);
			fprintf(stderr, "\nChecking . . .");
			for (count_1 = 0; count_1 < saved_count; count_1++) {
				if ((return_code = strlni_find(test_count, test_list,
