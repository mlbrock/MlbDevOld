...
/*
	...
	File Description	:	Implements Boyer-Moore string searching.
...
/*	BOH
	...
	NAME			:	BoyerMoore_mem_init_jump

	SYNOPSIS		:	jump_array_ptr = BoyerMoore_mem_init_jump(target_length,
	...
	DESCRIPTION	:	Initializes a character jump table to the state necessary
						for subsequent use by the first heuristic of the Boyer-Moore
	...
	PARAMETERS	:	Parameters to this function are as follow:
						...
						into which the Boyer-Moore jump data calculated by this
	...
	RETURNS		:	A pointer to the calculated Boyer-Moore jump array (that is,
						the ``jump_array`` parameter is returned).

	NOTES			:	

	CAVEATS		:	

	SEE ALSO		:	BoyerMoore_memi_init_jump
						BoyerMoore_mem_init_match
	...
	OUTPUT INDEX:	BoyerMoore_mem_init_jump
						...
						String Search Functions:BoyerMoore_mem_init_jump
						Boyer-Moore String Search Functions:BoyerMoore_mem_init_jump

	PUBLISH XREF:	BoyerMoore_mem_init_jump

	PUBLISH NAME:	BoyerMoore_mem_init_jump
...
/*	***********************************************************************	*/
#ifndef NARGS
unsigned int *BoyerMoore_mem_init_jump(unsigned int target_length,
	const void *target_area, unsigned int *jump_array)
#else
unsigned int *BoyerMoore_mem_init_jump(target_length, target_area, jump_array)
unsigned int  target_length;
const void   *target_area;
unsigned int *jump_array;
#endif /* #ifndef NARGS */
{
	...
	for (count_1 = 0; count_1 < target_length; count_1++)
		jump_array[((const unsigned char *) target_area)[count_1]] =
			(target_length - count_1) - 1;
...
/*	BOH
	...
	NAME			:	BoyerMoore_mem_init_match

	SYNOPSIS		:	match_array_ptr = BoyerMoore_mem_init_match(target_length,
	...
	DESCRIPTION	:	Initializes a character match table to the state necessary
						for subsequent use by the second heuristic of the
	...
	PARAMETERS	:	Parameters to this function are as follow:
						...
						into which the Boyer-Moore match data calculated by this
						function will be placed.
						...
						``BoyerMoore_mem_alloc_match`` will both allocate a memory
						area of the appropriate size and then call this function
						to initialize that area.

	RETURNS		:	A pointer to the calculated Boyer-Moore match array (that
						is, the ``match_array`` parameter is returned).

	NOTES			:	

	CAVEATS		:	

	SEE ALSO		:	BoyerMoore_memi_init_match
						BoyerMoore_mem_alloc_match
						BoyerMoore_mem_init_jump
	...
	OUTPUT INDEX:	BoyerMoore_mem_init_match
						...
						String Search Functions:BoyerMoore_mem_init_match
						Boyer-Moore String Search Functions:BoyerMoore_mem_init_match

	PUBLISH XREF:	BoyerMoore_mem_init_match

	PUBLISH NAME:	BoyerMoore_mem_init_match
...
/*	***********************************************************************	*/
#ifndef NARGS
unsigned int *BoyerMoore_mem_init_match(unsigned int target_length,
	const void *target_area, unsigned int *match_array)
#else
unsigned int *BoyerMoore_mem_init_match(target_length, target_area,
	match_array)
unsigned int  target_length;
const void   *target_area;
unsigned int *match_array;
	...
	if (target_length) {
		...
		while (count_1) {
			...
			while ((tmp_index_1 <= target_length) &&
				(((const unsigned char *) target_area)[count_1 - 1] !=
					((const unsigned char *) target_area)[tmp_index_1 - 1])) {
...
/*	BOH
	...
	NAME			:	BoyerMoore_mem_alloc_match

	SYNOPSIS		:	match_array_ptr = BoyerMoore_mem_alloc_match(target_length,
							target_area);
	...
	DESCRIPTION	:	Allocates and then initializes a character match table to
						the state necessary for subsequent use by the second
						heuristic of the Boyer-Moore string searching algorithm.
	...
	RETURNS		:	Returns from this function are as follow:
						...
						the allocated Boyer-Moore match array is returned.
	...
	SEE ALSO		:	BoyerMoore_memi_alloc_match
						BoyerMoore_mem_init_match
	...
	OUTPUT INDEX:	BoyerMoore_mem_alloc_match
						...
						String Search Functions:BoyerMoore_mem_alloc_match
						Boyer-Moore String Search Functions:BoyerMoore_mem_alloc_match

	PUBLISH XREF:	BoyerMoore_mem_alloc_match

	PUBLISH NAME:	BoyerMoore_mem_alloc_match
...
/*	***********************************************************************	*/
#ifndef NARGS
unsigned int *BoyerMoore_mem_alloc_match(unsigned int target_length,
	const void *target_area)
#else
unsigned int *BoyerMoore_mem_alloc_match(target_length, target_area)
unsigned int  target_length;
const void   *target_area;
#endif /* #ifndef NARGS */
{
	...
	return((target_length && ((match_array = ((unsigned int *)
		...
		NULL)) ? BoyerMoore_mem_init_match(target_length, target_area,
		match_array) : NULL);
}
/*	***********************************************************************	*/
...
/*	BOH
	...
	NAME			:	BoyerMoore_mem_search

	SYNOPSIS		:	found_ptr = BoyerMoore_mem_search(search_length,
	...
	DESCRIPTION	:	Initializes the tables necessary to both Boyer-Moore
						heuristics and uses those tables to locate the first
						occurrence of the memory area to which the ``target_area``
	...
	NOTES			:	This function initializes tables for both heuristics of the
						Boyer-Moore string searching algorithm. If you intend to
						perform multiple searchs using the same ``target_length``
						and ``target_area`` parameters, it will be much more
						efficient to initialize those tables once and to then use
						the related function ``BoyerMoore_mem_search_basic``.
	...
	SEE ALSO		:	BoyerMoore_mem_search_basic
						BoyerMoore_memi_search
	...
	OUTPUT INDEX:	BoyerMoore_mem_search
						...
						String Search Functions:BoyerMoore_mem_search
						Boyer-Moore String Search Functions:BoyerMoore_mem_search

	PUBLISH XREF:	BoyerMoore_mem_search

	PUBLISH NAME:	BoyerMoore_mem_search
...
/*	***********************************************************************	*/
#ifndef NARGS
void *BoyerMoore_mem_search(unsigned int search_length,
	const void *search_area, unsigned int  target_length,
	const void *target_area)
#else
void *BoyerMoore_mem_search(search_length, search_area, target_length,
	target_area)
unsigned int  search_length;
const void   *search_area;
unsigned int  target_length;
...
#endif /* #ifndef NARGS */
{
	...
	if (!target_length)
		...
		/*	***********************************************************	*/
		/*	***********************************************************	*/
		/*		Note that we don't check to see whether the allocation	*/
		/*	of 'match_array' failed. This is because the primitive		*/
		/*	function 'BoyerMoore_mem_search_basic()' will use only the	*/
		/*	first Boyer-Moore heuristic if 'match_array' is 'NULL'.		*/
		/*	***********************************************************	*/
		return_ptr = BoyerMoore_mem_search_basic(search_length, search_area,
			target_length, target_area, BoyerMoore_mem_init_jump(target_length,
			target_area, jump_array), match_array =
			BoyerMoore_mem_alloc_match(target_length, target_area));
		/*	***********************************************************	*/
		if (match_array != NULL)
			free(match_array);
	}
...
/*	BOH
	...
	NAME			:	BoyerMoore_mem_search_basic

	SYNOPSIS		:	found_ptr = BoyerMoore_mem_search_basic(search_length,
							search_area, target_length, target_area);
	...
	DESCRIPTION	:	Locates the first occurrence of the memory area to which
						...
						which the ``search_area`` parameter points using the
						Boyer-Moore string searching algorithm. The tables
						necessary for use by the two heuristics of the algorithm
						are assumed to have been initialized by the invoking
						function.
	...
	PARAMETERS	:	Parameters to this function are as follow:
						...
						(.) ``jump_array`` points to an array of ``unsigned int``
						which contains the Boyer-Moore jump data calculated by the
						invoking function.

						(.) ``match_array`` points to an array of ``unsigned int``
						which contains the Boyer-Moore match data calculated by the
						invoking function.
	...
	SEE ALSO		:	BoyerMoore_mem_search
	...
	OUTPUT INDEX:	BoyerMoore_mem_search_basic
						...
						String Search Functions:BoyerMoore_mem_search_basic
						Boyer-Moore String Search Functions:BoyerMoore_mem_search_basic

	PUBLISH XREF:	BoyerMoore_mem_search_basic

	PUBLISH NAME:	BoyerMoore_mem_search_basic
...
/*	***********************************************************************	*/
#ifndef NARGS
void *BoyerMoore_mem_search_basic(unsigned int search_length,
	const void *search_area, unsigned int target_length,
	const void *target_area, unsigned int *jump_array,
	unsigned int *match_array)
#else
void *BoyerMoore_mem_search_basic(search_length, search_area, target_length,
	target_area, jump_array, match_array)
unsigned int  search_length;
const void   *search_area;
unsigned int  target_length;
...
#endif /* #ifndef NARGS */
{
	...
	/*	If it isn't, don't call this function --- use 'strstr()' instead.	*/
	/*	*****************************************************************	*/
	else if (match_array != NULL) {
		while ((search_index < search_length) && target_index) {
			if (((const unsigned char *) search_area)[search_index] ==
				((const unsigned char *) target_area)[target_index - 1]) {
				search_index--;
				target_index--;
			}
			else {
				tmp_index_1   = jump_array[((const unsigned char *) search_area)
					[search_index]];
...
#endif /* #ifndef NARGS */
{
	...
	fprintf(stderr,
		"Test routine for function 'BoyerMoore_mem_search_basic()'\n");
	fprintf(stderr,
		"---- ------- --- -------- -------------------------------\n\n");
	...
	if (argc < 3) {
	...
	else if ((match_array = BoyerMoore_mem_alloc_match(target_length =
		...
		BoyerMoore_mem_init_jump(target_length, target_area, jump_array);
		...
		for (count_1 = 2; count_1 < ((unsigned int) argc); count_1++) {
			...
			while ((tmp_ptr = strstr(search_area, target_area)) != NULL) {
			...
			check_sum_2   = 0L;
			SDTIF_GetTimeUSecs(&start_time);
			while ((tmp_ptr = ((char *) BoyerMoore_mem_search_basic(search_length,
			...
			if ((found_count_0 != found_count_1) ||
				...
				fprintf(stderr, "strstr                           : %10u %10lu\n",
					found_count_0, check_sum_0);
				fprintf(stderr, "memmem                           : %10u %10lu\n",
		...
		fprintf(stderr, "Instances found                  : %10lu\n",
			total_found_count);
		fprintf(stderr, "---------------------------------- ----------\n");
		fprintf(stderr, "strstr() seconds required        : %10.6f\n",
			((double) total_time_0) / 1000000.0);
		fprintf(stderr, "memmem() seconds required        : %10.6f\n",
...
# if 0
#  ifndef NARGS
char *NEW_strstr(const char *search_string, const char *target_string)
#  else
char *NEW_strstr(search_string, target_string)
...
#  endif /* #ifndef NARGS */
{
	...
	if (!(*target_string))
		...
		for ( ;(search_string = strchr(search_string, *target_string)) != NULL;
			search_string++) {
			for (tmp_ptr_1 = search_string, tmp_ptr_2 = target_string; ; ) {
				if (!(*(++tmp_ptr_2)))
					return(((char *) search_string));
				else if (*(++tmp_ptr_1) != *tmp_ptr_2)
					break;
			}
		}
	}
