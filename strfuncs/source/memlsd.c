...
/*
	...
	File Description	:	Array of pointers search delta logic.
...
/* BOH
   ...
   NAME        :  meml_search_delta

   SYNOPSIS    :  return_code = meml_search_delta(in_count_1, in_list_1,
   ...
   DESCRIPTION :  Determines the delta between the elements in two arrays
						...
						``MEM_memcmp`` as a comparison functions.
   ...
   NOTES       :  This function may allocate memory for ``uniq_list_1``,
						...
						To compare arrays of pointers to structures, you should
						probably use the function ``MEM_LIST_search_delta``.

   CAVEATS     :  The results of invoking this function with an ``in_list_1``
						or ``in_list_2`` not in sorted order are undefined.

   SEE ALSO    :  memli_search_delta
   ...
   OUTPUT INDEX:  meml_search_delta
						Memory List Search Delta Functions:meml_search_delta
						STRFUNCS:Memory List Search Delta Functions:meml_search_delta
						String Functions:See STRFUNCS

   PUBLISH XREF:  meml_search_delta

   PUBLISH NAME:	meml_search_delta
...
/* *********************************************************************** */
#ifndef NARGS
int meml_search_delta(unsigned int in_count_1, void **in_list_1,
	unsigned int in_count_2, void **in_list_2, unsigned int *uniq_count_1,
	unsigned int **uniq_list_1, unsigned int *uniq_count_2,
	unsigned int **uniq_list_2, unsigned int *same_count_1,
	unsigned int **same_list_1, unsigned int *same_count_2,
	unsigned int **same_list_2, size_t data_length)
#else
int meml_search_delta(in_count_1, in_list_1, in_count_2, in_list_2,
...
#endif /* #ifndef NARGS */
{
	...
	return(MEM_LIST_search_delta(NULL, in_count_1, in_list_1, in_count_2,
...
#endif /* #ifndef NARGS */
{
	...
	fprintf(stderr, "Test of function 'meml_search_delta()'\n");
	fprintf(stderr, "---- -- -------- ---------------------\n\n");
	...
	else if ((return_code = meml_search_delta(file_count_1, file_list_1,
