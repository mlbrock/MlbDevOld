...
/*
	...
	File Description	:	Case-insensitive array of pointers non-search delta
								logic.
...
/* BOH
   ...
   DESCRIPTION :  Determines the delta between the elements in two arrays
						...
						comparison function ``MEM_memicmp`` returns a non-zero
						value.
   ...
   NOTES       :  This function may allocate memory for ``uniq_list_1``,
						...
						To compare arrays of pointers structures, you should
						probably use the function ``MEM_LIST_delta``.
   ...
   OUTPUT INDEX:  memli_delta
						Memory List Delta Functions:memli_delta
						STRFUNCS:Memory List Delta Functions:memli_delta
						String Functions:See STRFUNCS

   PUBLISH XREF:  memli_delta

   PUBLISH NAME:	memli_delta

	ENTRY CLASS	:	Memory List Delta Functions

EOH */
/* *********************************************************************** */
#ifndef NARGS
int memli_delta(unsigned int string_count_1, void **string_list_1,
...
#else
int memli_delta(string_count_1, string_list_1, string_count_2, string_list_2,
...
#endif /* #ifndef NARGS */
{
	return(MEM_LIST_delta(NULL, string_count_1, ((void **) string_list_1),
		...
		same_count, same_list, data_length, MEM_memicmp));
}
/* *********************************************************************** */
...
#endif /* #ifndef NARGS */
{
	...
	fprintf(stderr, "Test of function 'memli_delta()'\n");
	fprintf(stderr, "---- -- -------- ---------------\n\n");
	...
	if ((return_code = GetFileList(argv[1], &file_count_1, &file_list_1,
	...
	else if ((return_code = memli_delta(file_count_1, file_list_1, file_count_2,
		...
		for (count_1 = 0; count_1 < diff_count; count_1++) {
			printf("[%05u][%05u][%s]\n", count_1, diff_list[count_1],
				((char *) file_list_1[diff_list[count_1]]));
			printf("       [%05u][%s]\n", diff_list[count_1],
				((char *) file_list_2[diff_list[count_1]]));
		}
		printf("-----------------------------------------------------------\n");
		...
		for (count_1 = 0; count_1 < same_count; count_1++) {
			printf("[%05u][%05u][%s]\n", count_1, same_list[count_1],
				((char *) file_list_1[same_list[count_1]]));
			printf("       [%05u][%s]\n", same_list[count_1],
				((char *) file_list_2[same_list[count_1]]));
		}
		printf("-----------------------------------------------------------\n");
	}
