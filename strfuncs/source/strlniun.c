...
/* BOH
   ...
   SYNOPSIS    :  strlni_uniq(string_count, string_list, string_length);
   ...
   OUTPUT INDEX:  strlni_uniq
						String List Functions:strlni_uniq
						STRFUNCS:String List Functions:strlni_uniq
						String Functions:See STRFUNCS

   PUBLISH XREF:  strlni_uniq

   PUBLISH NAME:	strlni_uniq
...
/* *********************************************************************** */
#ifndef NARGS
void strlni_uniq(unsigned int *string_count, char ***string_list,
	size_t string_length)
#else
void strlni_uniq(string_count, string_list, string_length)
unsigned int   *string_count;
char         ***string_list;
size_t          string_length;
#endif /* #ifndef NARGS */
...
int main()
{
	...
	fprintf(stderr, "Test of function 'strlni_uniq()'\n");
	fprintf(stderr, "---- -- -------- ---------------\n\n");
	...
	if (!return_code) {
		...
		strlni_uniq(&test_count, &test_list, max_length);
		fprintf(stderr, "[HEAD] ---> [%08p]\n", test_list);
		for (count_1 = 0; count_1 < test_count; count_1++)
			fprintf(stderr, "[%05u][%08p] ---> [%s]\n", count_1,
				test_list[count_1], test_list[count_1]);
