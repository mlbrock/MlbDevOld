...
/*
	...
	File Description	:	Basic array of pointers search delta logic.
...
/* BOH
   ...
   NAME        :  MEM_LIST_search_delta

   SYNOPSIS    :  return_code = MEM_LIST_search_delta(control_ptr, in_count_1,
							in_list_1, in_count_2, in_list_2, uniq_count_1,
							uniq_list_1, uniq_count_2, uniq_list_2, diff_count_1,
							diff_list_1, diff_count_2, diff_list_2, same_count_1,
   ...
   DESCRIPTION :  Determines the delta between the elements in two arrays
						of pointers by performing binary searches using
						user-supplied comparison functions.
   ...
   PARAMETERS  :  Parameters to this function are as follow:
						...
						the arrays of pointers ``in_list_1`` and ``in_list_2``.
   ...
   OUTPUT INDEX:  MEM_LIST_search_delta
						Memory List Search Delta Functions:MEM_LIST_search_delta
						STRFUNCS:Memory List Search Delta Functions:MEM_LIST_search_delta
						String Functions:See STRFUNCS

   PUBLISH XREF:  MEM_LIST_search_delta

   PUBLISH NAME:	MEM_LIST_search_delta

	ENTRY CLASS	:	Memory List Search Delta Functions

EOH */
/* *********************************************************************** */
#ifndef NARGS
int MEM_LIST_search_delta(void *control_ptr, unsigned int in_count_1,
...
#else
int MEM_LIST_search_delta(control_ptr, in_count_1, in_list_1, in_count_2,
	...
	if (in_count_1 || in_count_2) {
		...
		if ((!in_count_1) || (!in_count_2)) {
			...
			for (count_1 = 0; count_1 < in_count_1; count_1++) {
				if (STR_LIST_bsearch(control_ptr, in_count_2, in_list_2,
					in_list_1[count_1], data_length, &found_index, 1,
					compare_func_1) == STRFUNCS_ITEM_FOUND) {
					if ((compare_func_2 != NULL) &&
						(compare_func_1 != compare_func_2) &&
						(*compare_func_2)(control_ptr, in_list_1[count_1],
						in_list_2[found_index], data_length)) {
			...
			if (return_code == STRFUNCS_SUCCESS) {
				for (count_1 = 0; count_1 < in_count_2; count_1++) {
					if (STR_LIST_bsearch(control_ptr, in_count_1, in_list_1,
						in_list_2[count_1], data_length, &found_index, 1,
						compare_func_1) == STRFUNCS_ITEM_NOT_FOUND)
...
#endif /* #ifndef NARGS */
{
	...
	fprintf(stderr, "Test of function 'MEM_LIST_search_delta()'\n");
	fprintf(stderr, "---- -- -------- -------------------------\n\n");
	...
	if ((return_code = GetFileList(argv[1], &file_count_1, &file_list_1,
...
#ifndef NARGS
	else if ((return_code = MEM_LIST_search_delta(NULL, file_count_1,
		...
		for (count_1 = 0; count_1 < uniq_count_1; count_1++)
			printf("[%05u][%05u][%s]\n", count_1, uniq_list_1[count_1],
				((char *) file_list_1[uniq_list_1[count_1]]));
		printf("-----------------------------------------------------------\n");
		printf("Unique to file '%s' --- %u elements:\n", argv[2],
			uniq_count_2);
		for (count_1 = 0; count_1 < uniq_count_2; count_1++)
			printf("[%05u][%05u][%s]\n", count_1, uniq_list_2[count_1],
				((char *) file_list_2[uniq_list_2[count_1]]));
		printf("-----------------------------------------------------------\n");
		...
		for (count_1 = 0; count_1 < diff_count_1; count_1++) {
			printf("[%05u][%05u][%s]\n", count_1, diff_list_1[count_1],
				((char *) file_list_1[diff_list_1[count_1]]));
			printf("       [%05u][%s]\n", diff_list_2[count_1],
				((char *) file_list_2[diff_list_2[count_1]]));
		}
		printf("-----------------------------------------------------------\n");
		...
		for (count_1 = 0; count_1 < same_count_1; count_1++) {
			printf("[%05u][%05u][%s]\n", count_1, same_list_1[count_1],
				((char *) file_list_1[same_list_1[count_1]]));
			printf("       [%05u][%s]\n", same_list_2[count_1],
				((char *) file_list_2[same_list_2[count_1]]));
		}
		printf("-----------------------------------------------------------\n");
	}
	...
	if ((file_ptr = fopen(file_name, "r")) == NULL) {
		...
		while ((!feof(file_ptr)) && (!ferror(file_ptr))) {
			*buffer = '\0';
			...
			if ((return_code = meml_append(file_count, file_list, buffer,
		...
		if (!return_code)
#ifndef NARGS
			STR_LIST_qsort(NULL, *file_count, *file_list, DATA_LENGTH,
				((int (*)(void *, const void *, const void *, size_t))
				STR_strcmp));
#else
			STR_LIST_qsort(NULL, *file_count, *file_list, DATA_LENGTH,
