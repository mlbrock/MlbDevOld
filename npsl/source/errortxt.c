...
#ifdef _Windows
static const GEN_ERROR_SPEC NPSL_ErrorList[] = {
	{	WSAEACCES,	"WSAEACCES",
		"Permission denied",
...
/*	***********************************************************************	*/
void NPSL_GetErrorList(unsigned int *out_count, const GEN_ERROR_SPEC **out_list)
{
#ifdef _Windows
...
{
#ifdef _Windows
	const GEN_ERROR_SPEC *errno_ptr;
	char                  buffer[4196];
...
EOH */
/*	***********************************************************************	*/
const GEN_ERROR_SPEC *NPSL_GetErrorPtr(int errno_code)
{
#ifdef _Windows
	unsigned int          count_1;
	const GEN_ERROR_SPEC *errno_ptr = NULL;
...
/*	***********************************************************************	*/
#ifndef NARGS
const GEN_ERROR_SPEC *NPSL_GetLastErrorPtr(void)
#else
const GEN_ERROR_SPEC *NPSL_GetLastErrorPtr()
#endif /* #ifndef NARGS */
{
#ifdef _Windows
...
int main(int argc, char **argv)
{
	...
	const GEN_ERROR_SPEC *error_list;
	unsigned int          length;
	...
	for (count_1 = 0; count_1 < error_count; count_1++) {
		length    = strlen(NPSL_GetErrorString(error_list[count_1].error_code,
			0, 0, error_text));
