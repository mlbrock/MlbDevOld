	...
	unsigned int *out_host_count, char ***out_host_list, char *error_text)
{
	...
	*out_host_list  = NULL;
	...
	for (count_1 = 0; count_1 < in_host_count; count_1++) {
		...
		else if (strl_append(out_host_count, out_host_list,
			hent_data.h_name) != STRFUNCS_SUCCESS) {
			sprintf(error_text, "%s '%-.300s' to the host name list.",
				"Unable to add the host name alias", hent_data.h_name);
			return_code = NPSL_MEMORY_FAILURE;
			break;
		}
		this_ptr = hent_data.h_aliases;
		while (*this_ptr != NULL) {
