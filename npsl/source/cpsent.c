	...
	char *error_text)
{
	...
	required_size = sizeof(NPSL_SERVENT) +
		strlen(in_serv_ent_ptr->s_name) + 1 + (alias_count * sizeof(char *)) +
		strlen(in_serv_ent_ptr->s_proto) + 1 + alias_length;
	...
	memcpy(str_ptr, in_serv_ent_ptr->s_name,
		str_length = (strlen(in_serv_ent_ptr->s_name) + 1));
	str_ptr += str_length;

	tmp_serv_ent.s_proto = str_ptr;
	memcpy(str_ptr, in_serv_ent_ptr->s_proto,
		str_length = (strlen(in_serv_ent_ptr->s_proto) + 1));
	str_ptr += str_length;
	...
	for (count_1 = 0; count_1 < alias_count; count_1++) {
		if (in_serv_ent_ptr->s_aliases[count_1] != NULL) {
			...
			memcpy(str_ptr, in_serv_ent_ptr->s_aliases[count_1],
				str_length = (strlen(in_serv_ent_ptr->s_aliases[count_1]) + 1));
			str_ptr += str_length;
		}
		else
	...
	NPSL_SERVENT *out_serv_ent_ptr, char *error_text)
{
	...
	if ((in_serv_ent_ptr->s_name != NULL) && ((out_serv_ent_ptr->s_name =
		...
		STR_AllocMsgItem(strlen(in_serv_ent_ptr->s_name) + 1, error_text,
			"Unable to copy the 'NPSL_SERVENT' member 's_name'");
		...
		STR_AllocMsgItem(strlen(in_serv_ent_ptr->s_proto) + 1, error_text,
			"Unable to copy the 'NPSL_SERVENT' member 's_proto'");
		return_code = NPSL_MEMORY_FAILURE;
	}
