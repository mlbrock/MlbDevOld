	...
	char *error_text)
{
	...
	required_size = sizeof(NPSL_PROTOENT) +
		strlen(in_proto_ent_ptr->p_name) + 1 + (alias_count * sizeof(char *)) +
		alias_length;
	...
	memcpy(str_ptr, in_proto_ent_ptr->p_name,
		str_length = (strlen(in_proto_ent_ptr->p_name) + 1));
	str_ptr += str_length;
	...
	for (count_1 = 0; count_1 < alias_count; count_1++) {
		if (in_proto_ent_ptr->p_aliases[count_1] != NULL) {
			...
			memcpy(str_ptr, in_proto_ent_ptr->p_aliases[count_1],
				str_length = (strlen(in_proto_ent_ptr->p_aliases[count_1]) + 1));
			str_ptr += str_length;
		}
		else
	...
	NPSL_PROTOENT *out_proto_ent_ptr, char *error_text)
{
	...
	if ((in_proto_ent_ptr->p_name != NULL) && ((out_proto_ent_ptr->p_name =
		...
		STR_AllocMsgItem(strlen(in_proto_ent_ptr->p_name) + 1, error_text,
			"Unable to copy the 'NPSL_PROTOENT' member 'p_name'");
