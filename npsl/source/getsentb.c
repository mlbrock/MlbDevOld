	...
	unsigned int *required_length, char *error_text)
{
	...
	else if (str_digit(serv_port_string, 0) != NULL) {
		if (error_text != NULL)
			strcat(nstrcat(strcat(strcpy(error_text,
	...
	char *error_text)
{
	...
	if ((serv_proto != NULL) && (!(*serv_proto))) {
		...
		if ((tmp_ent_ptr = getservbyport_r(serv_port, serv_proto,
...
#else
		if ((tmp_ent_ptr = getservbyport(serv_port, serv_proto)) == NULL) {
			if (error_text != NULL) {
				sprintf(error_text,
					"%s '%s()' for service port '%d', protocol '%-.127s': ",
