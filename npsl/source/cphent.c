	...
	char *error_text)
{
	...
	while (*in_alias != NULL) {
		alias_length += ((unsigned int) strlen(*in_alias)) + 1;
		in_alias++;
		alias_count++;
	}
	...
	required_size = sizeof(NPSL_HOSTENT) +
		strlen(in_host_ent_ptr->h_name) + 1 + (alias_count * sizeof(char *)) +
		(address_count * sizeof(char *)) + alias_length + address_length;
	...
	memcpy(str_ptr, in_host_ent_ptr->h_name,
		str_length = (strlen(in_host_ent_ptr->h_name) + 1));
	str_ptr += str_length;
	...
	for (count_1 = 0; count_1 < alias_count; count_1++) {
		if (in_host_ent_ptr->h_aliases[count_1] != NULL) {
			...
			memcpy(str_ptr, in_host_ent_ptr->h_aliases[count_1],
				str_length = (strlen(in_host_ent_ptr->h_aliases[count_1]) + 1));
			str_ptr += str_length;
		}
		else
	...
	NPSL_HOSTENT *out_host_ent_ptr, char *error_text)
{
	...
	while (*in_alias != NULL) {
		alias_length += strlen(*in_alias) + 1;
		in_alias++;
		alias_count++;
	}
	...
	if ((in_host_ent_ptr->h_name != NULL) && ((out_host_ent_ptr->h_name =
		...
		STR_AllocMsgItem(strlen(in_host_ent_ptr->h_name) + 1, error_text,
			"Unable to copy the 'NPSL_HOSTENT' member 'h_name'");
		return_code = NPSL_MEMORY_FAILURE;
	}
	else if (meml_allocate(&tmp_address_count,
		((void ***) &out_host_ent_ptr->h_addr_list), 0, address_count) !=
      STRFUNCS_SUCCESS) {
		STR_AllocMsgPtrList(address_count, address_length, error_text,
			"Unable to copy the 'NPSL_HOSTENT' member 'h_addr_list' pointer list");
		return_code = NPSL_MEMORY_FAILURE;
	}
	else if (meml_allocate(&tmp_alias_count,
		((void ***) &out_host_ent_ptr->h_aliases), 0, alias_count) !=
		STRFUNCS_SUCCESS) {
		STR_AllocMsgPtrList(alias_count, alias_length, error_text,
			"Unable to copy the 'NPSL_HOSTENT' member 'h_aliases' pointer list");
		return_code = NPSL_MEMORY_FAILURE;
	}
	else {
		for (count_1 = 0; count_1 < (address_count - 1); count_1++) {
			if ((out_host_ent_ptr->h_addr_list[count_1] =
				((char *) memdup(in_host_ent_ptr->h_addr_list[count_1],
				((unsigned int) in_host_ent_ptr->h_length)))) == NULL) {
				STR_AllocMsgPtrList(address_count, address_length, error_text,
