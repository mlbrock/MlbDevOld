...
/*
	...
	File Description	:	Reads raw buffers using the 'FILE' facility.
...
/*	***********************************************************************	*/
#ifndef NARGS
int TFREAD_ReadBufferRawFILE(const char *file_name,
	unsigned int *buffer_length, char **buffer_ptr, char *error_text)
#else
int TFREAD_ReadBufferRawFILE(file_name, buffer_length, buffer_ptr, error_text)
...
#endif /* #ifndef NARGS */
{
	...
	return(TFREAD_ReadBufferRawBasic(TFREAD_GetDefDataFILE(&cfile_funcs),
