...
/* *********************************************************************** */
COMPAT_FN_DECL_STATIC(int DBC_MSSQL_LoadTableNames, (void *dbc_control_ptr,
...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetTableName(spec_string, in_database_name, name_count, name_list,
	error_text)
...
char          *error_text;
{
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetTableNames",
		...
		return_code = DBC_MSSQL_GetTableNameBasic(dbc_context_data.dbc_control_ptr,
...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetTableNameBasic(dbc_control_ptr, in_database_name, name_count,
...
char          *error_text;
{
	return(DBC_MSSQL_LoadTableNames(dbc_control_ptr, in_database_name, DBC_FALSE,
...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetUserTableName(spec_string, in_database_name, name_count,
	name_list, error_text)
const char    *spec_string;
const char    *in_database_name;
unsigned int  *name_count;
...
char          *error_text;
{
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetUserTableNames",
		spec_string, NULL, DBC_FLAG_NONE, NULL, error_text)) == DBC_SUCCESS) {
		return_code =
			DBC_MSSQL_GetUserTableNameBasic(dbc_context_data.dbc_control_ptr,
			in_database_name, name_count, name_list, error_text);
		DBC_Close(&dbc_context_data);
	}
	else {
...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetUserTableNameBasic(dbc_control_ptr, in_database_name,
	name_count, name_list, error_text)
void          *dbc_control_ptr;
const char    *in_database_name;
unsigned int  *name_count;
...
char          *error_text;
{
	return(DBC_MSSQL_LoadTableNames(dbc_control_ptr, in_database_name, DBC_TRUE,
		name_count, name_list, error_text));
}
/*	***********************************************************************	*/

/* *********************************************************************** */
static int DBC_MSSQL_LoadTableNames(dbc_control_ptr, in_database_name,
...
char          *error_text;
{
	...
	*name_list  = NULL;
	...
	if ((return_code = DBC_MSSQL_BreakDatabaseName(in_database_name,
		...
		" AND uid IN (SELECT uid FROM %s.dbo.sysusers WHERE (name = \"%s\"))",
			...
			return_code = DBC_MSSQL_GetNameListBasic(dbc_control_ptr, sql_command,
...
char **argv;
{
	...
	fprintf(stderr, "Test routine for 'DBC_MSSQL_GetTableName()'\n");
	fprintf(stderr, "---- ------- --- ------------------------\n");
	...
	for (count_1 = 2; count_1 < argc; count_1++) {
		fprintf(stderr, "Getting table list for database '%s' --- ",
			argv[count_1]);
		if ((return_code = DBC_MSSQL_GetTableName(argv[1], argv[count_1],
	...
	for (count_1 = 2; count_1 < argc; count_1++) {
		fprintf(stderr, "Getting user table list for database '%s' --- ",
			argv[count_1]);
		if ((return_code = DBC_MSSQL_GetUserTableName(argv[1], argv[count_1],
			&name_count, &name_list, error_text)) != DBC_SUCCESS)
			goto EXIT_FUNCTION;
