...
/*	BOH
   ...
   SYNOPSIS    :	return_code = DBC_MSSQL_Open(dbc_control_ptr, app_name,
   ...
   OUTPUT INDEX:	DBC_MSSQL_Open
						DBC DB-LIB Functions:DBC_MSSQL_Open
						DBC Functions:Connect Functions:DBC_MSSQL_Open

   PUBLISH XREF:	DBC_MSSQL_Open

   PUBLISH NAME:	DBC_MSSQL_Open

	ENTRY CLASS	:	DBC Functions:Connect Functions

EOH */
/*	***********************************************************************	*/
int DBC_MSSQL_Open(dbc_context_ptr, conn_type, conn_string, mlog_ptr, flags,
...
char        *error_text;
{
	...
	else if ((return_code = DBC_MSSQL_Initialize(dbc_context_ptr, mlog_ptr,
		flags, user_data_ptr, error_text)) != DBC_SUCCESS)
		goto EXIT_FUNCTION;

	dbc_control_ptr = dbc_context_ptr->dbc_control_ptr;
	...
	if (!(*dbc_control_ptr->conn_list[DBC_MSSQL_SERVER_NAME])) {
	...
	if ((return_code = DBC_MSSQL_MSG_DefaultHandler(dbc_control_ptr,
		...
		if ((dbc_control_ptr->server_ptr = dblogin()) == NULL) {
			sprintf(error_text,
				...
				dbc_control_ptr->conn_list[DBC_MSSQL_SERVER_NAME],
			...
			/*	********************************************************
				...
				********************************************************
				...
				from Microsoft SQL Server 4.9.2 through System 10. In Microsoft SQL Server 11,
				however, it trashes the LOGINREC.
				DBSETLPACKET(dbc_control_ptr->server_ptr, DB__MAX_PACKET_SIZE);
				********************************************************	*/
			if (*dbc_control_ptr->conn_list[DBC_MSSQL_USER_NAME])
				DBSETLUSER(dbc_control_ptr->server_ptr,
					dbc_control_ptr->conn_list[DBC_MSSQL_USER_NAME]);
			if (*dbc_control_ptr->conn_list[DBC_MSSQL_PASS_WORD])
				DBSETLPWD(dbc_control_ptr->server_ptr,
				dbc_control_ptr->conn_list[DBC_MSSQL_PASS_WORD]);
			...
			"%s %s database '%s' for owner '%s' on server '%s' failed: %-.800s",
					...
					(*dbc_control_ptr->conn_list[DBC_MSSQL_DB_NAME]) ?
					dbc_control_ptr->conn_list[DBC_MSSQL_DB_NAME] : "?",
					(*dbc_control_ptr->conn_list[DBC_MSSQL_OWNER_NAME]) ?
					dbc_control_ptr->conn_list[DBC_MSSQL_OWNER_NAME] : "?",
					dbc_control_ptr->conn_list[DBC_MSSQL_SERVER_NAME],
					DBC_MSSQL_MSG_GetHandlerMessage(error_buffer));
			...
			/*	*****************************************************
				...
				*****************************************************
			else if (*dbc_control_ptr->conn_list[DBC_MSSQL_DB_NAME] &&
				(dbuse(dbc_control_ptr->database_ptr,
				dbc_control_ptr->conn_list[DBC_MSSQL_DB_NAME]) != SUCCEED)) {
				sprintf(error_text,
			"%s %s database '%s' for owner '%s' on server '%s' failed: %-.800s",
...
EXIT_FUNCTION:
	...
	if ((return_code != DBC_SUCCESS) && (dbc_control_ptr != NULL)) {
		if (was_initialized_flag == DBC_FALSE) {
			DBC_MSSQL_Close(&dbc_control_ptr);
