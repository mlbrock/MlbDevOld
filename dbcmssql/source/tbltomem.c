...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_TableToMembers(dbc_control_ptr, in_table_name, structure_size,
	...
	*member_list  = NULL;
	...
	if ((return_code = DBC_MSSQL_GetColumnBasic(dbc_control_ptr, in_table_name,
		...
		if (!column_count) {
			...
			return_code = DBC_MSSQL_ColumnsToMembers(column_count, column_list,
...
char **argv;
{
	...
	fprintf(stderr, "Test routine for 'DBC_MSSQL_TableToMembers()'\n");
	fprintf(stderr, "---- ------- --- --------------------------\n");
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetCols",
		...
		for (count_1 = 2; count_1 < argc; count_1++) {
			...
			if ((return_code =
				DBC_MSSQL_TableToMembers(dbc_context_data.dbc_control_ptr,
