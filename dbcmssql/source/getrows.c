...
/*
	...
	File Description	:	Manages the retrieval of rows from a Microsoft
								SQL Server database.
...
/*	BOH
   ...
   NAME        :	DBC_MSSQL_GetRows

   SYNOPSIS    :	return_code = DBC_MSSQL_GetRows(dbc_control_ptr,
   ...
   DESCRIPTION :	Sends an SQL statement to a Microsoft SQL Server database server and
   ...
   PARAMETERS  :	Parameters to this function are as follow:
						...
						rows received from the Microsoft SQL Server server.
						...
						have rows received from Microsoft SQL Server embedded within a larger
						...
						return all rows sent by the Microsoft SQL Server server.
						...
						amount of memory to hold the rows received from Microsoft SQL Server
						before returning.
   ...
   OUTPUT INDEX:	DBC_MSSQL_GetRows
						DBC DB-LIB Functions:DBC_MSSQL_GetRows
						DBC Functions:Row Retrieval Functions:DBC_MSSQL_GetRows

   PUBLISH XREF:	DBC_MSSQL_GetRows

   PUBLISH NAME:	DBC_MSSQL_GetRows

	ENTRY CLASS	:	DBC Functions:Row Retrieval Functions

EOH */
/*	***********************************************************************	*/
int DBC_MSSQL_GetRows(dbc_control_ptr, member_count, member_list, sql_command,
...
char               *error_text;
{
	...
	*out_row_list              = NULL;
	...
	return(DBC_MSSQL_GetRowsBasic(dbc_control_ptr, &get_rows, out_row_count,
		out_row_list, error_text));
#else
	CODE NOTE: To be removed.
	int        return_code  = DBC_DB_FAILURE;
	void      *bind_buffer  = NULL;
	int        handler_flag = DBC_FALSE;
	RETCODE    db_results_flag;
	...
	*out_row_list     = NULL;
	...
	alloc_granularity = (!alloc_granularity) ? 1 : alloc_granularity;
	...
	else if (!out_row_size) {
		strcpy(error_text, "Total row size is zero (0).");
		return_code = DBC_FAILURE;
		goto EXIT_FUNCTION;
	}
	else if (!data_row_size) {
		strcpy(error_text, "Data row size is zero (0).");
		return_code = DBC_FAILURE;
		goto EXIT_FUNCTION;
	}
	else if (out_row_size < data_row_size) {
		sprintf(error_text,
			"The total row size (%u) is less than the data row size (%u).",
			out_row_size, data_row_size);
		return_code = DBC_FAILURE;
		goto EXIT_FUNCTION;
	}
	else if (data_row_offset >= out_row_size) {
		sprintf(error_text,
			"The data row offset (%u) is not less than the total row size (%u).",
			data_row_offset, out_row_size);
		return_code = DBC_FAILURE;
		goto EXIT_FUNCTION;
	}
	else if ((data_row_offset + data_row_size) > out_row_size) {
		sprintf(error_text, "%s (%u) %s (%u) %s (%u) --- %u.",
			"The total row size", out_row_size,
			"is less than the sum of the data row offset", data_row_offset,
			"and the data row size", data_row_size,
			data_row_offset + data_row_size);
		return_code = DBC_FAILURE;
		goto EXIT_FUNCTION;
	}

	if ((bind_buffer = calloc(out_row_size, sizeof(char))) == NULL) {
		sprintf(error_text, "%s (%u bytes required).",
			"Unable to allocate temporary database row binding area",
			out_row_size * sizeof(char));
	...
	handler_flag = DBC_TRUE;

	if ((return_code = DBC_MSSQL_DoSQLCommand(dbc_control_ptr, sql_command,
		error_text)) != DBC_SUCCESS)
		goto EXIT_FUNCTION;

	if ((db_results_flag = dbresults(dbc_control_ptr->database_ptr)) ==
		FAIL) {
		strcat(strcpy(error_text, "Call to 'dbresults()' failed --- "),
			DBC_MSSQL_MSG_GetHandlerMessage(error_buffer));
		dbcancel(dbc_control_ptr->database_ptr);
		dbcanquery(dbc_control_ptr->database_ptr);
		return_code = DBC_FAILURE;
		goto EXIT_FUNCTION;
	}
	else if (db_results_flag == NO_MORE_RESULTS) {
		return_code = DBC_SUCCESS;
		goto EXIT_FUNCTION;
	}

	if ((return_code = DBC_MSSQL_BindMembers(dbc_control_ptr, member_count,
		member_list, table_name, bind_buffer, error_text)) != DBC_SUCCESS) {
		dbcancel(dbc_control_ptr->database_ptr);
		dbcanquery(dbc_control_ptr->database_ptr);
	}
	else {
		*error_buffer  = '\0';
		old_alloc_size = 0;
		while (dbnextrow(dbc_control_ptr->database_ptr) != NO_MORE_ROWS) {
			if (!(*out_row_count % alloc_granularity)) {
				new_alloc_size = ((*out_row_count / alloc_granularity) + 1) *
					alloc_granularity * out_row_size;
				if ((tmp_ptr = ((*out_row_list == NULL) ?
					calloc(new_alloc_size, sizeof(char)) :
					realloc(*out_row_list, new_alloc_size * sizeof(char)))) !=
					NULL)
					*out_row_list = tmp_ptr;
				else {
					STR_AllocMsgList(new_alloc_size / out_row_size, out_row_size,
						error_text,
			...
			memcpy(((char *) *out_row_list) + ((*out_row_count)++ *
				out_row_size), bind_buffer, out_row_size);
			if (max_row_count && (*out_row_count == max_row_count)) {
				dbcancel(dbc_control_ptr->database_ptr);
				dbcanquery(dbc_control_ptr->database_ptr);
				break;
			}
	...
	if ((return_code == DBC_SUCCESS) && (*out_row_count % alloc_granularity)) {
		if ((tmp_ptr = realloc(*out_row_list, *out_row_count * out_row_size)) !=
			NULL)
			*out_row_list = tmp_ptr;
		else {
			sprintf(error_text,
				...
				old_alloc_size, *out_row_count * out_row_size,
				"row count", *out_row_count, "row size", out_row_size,
				*out_row_count * out_row_size);
			return_code = DBC_MEMORY_FAILURE;
		}
	}
...
*/
/*	***********************************************************************	*/
int DBC_MSSQL_GetRowsBasic(dbc_control_ptr, get_rows_ptr, out_row_count,
	out_row_list, error_text)
DBC_MSSQL_CONTROL  *dbc_control_ptr;
...
char               *error_text;
{
	...
	*out_row_list                  = NULL;
	...
	if ((return_code = DBC_MSSQL_DoSQLCommand(dbc_control_ptr,
	...
	if ((return_code = DBC_MSSQL_BindMembers(dbc_control_ptr,
	...
	*error_buffer  = '\0';
	...
	while (dbnextrow(dbc_control_ptr->database_ptr) != NO_MORE_ROWS) {
		...
		memset(bind_buffer, '\0', tmp_get_rows.out_row_size);
