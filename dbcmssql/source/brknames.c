...
/*	***********************************************************************	*/
COMPAT_FN_DECL_STATIC(int DBC_MSSQL_BreakNames,
	(const char *name_type, const char *in_name, char *out_database_name,
	char *out_owner_name, char *out_table_name, char *error_text));
/*	***********************************************************************	*/

/*	***********************************************************************	*/
int DBC_MSSQL_BreakDatabaseName(in_name, out_database_name, out_owner_name,
	error_text)
const char *in_name;
...
char       *error_text;
{
	...
	char          tmp_in_name[((DBC_MSSQL_MAX_ANY_NAME_LEN * 3) + 2) + 1];
	char          tmp_out_name[DBC_MSSQL_MAX_ANY_NAME_LEN + 1];

	if ((in_name != NULL) && *in_name &&
		(strlen(in_name) < ((DBC_MSSQL_MAX_ANY_NAME_LEN * 2) + 2)) &&
	...
	return(DBC_MSSQL_BreakNames("database", tmp_in_ptr, out_database_name,
		out_owner_name, tmp_out_name, error_text));
}
/*	***********************************************************************	*/

/*	***********************************************************************	*/
int DBC_MSSQL_BreakTableName(in_name, out_database_name, out_owner_name,
...
char       *error_text;
{
	return(DBC_MSSQL_BreakNames("table", in_name, out_database_name,
		out_owner_name, out_table_name, error_text));
}
/*	***********************************************************************	*/

/*	***********************************************************************	*/
static int DBC_MSSQL_BreakNames(name_type, in_name, out_database_name,
	out_owner_name, out_table_name, error_text)
...
char       *error_text;
{
	...
	char         *tmp_ptr[3];
	unsigned int  point_count;
	char          tmp_name[((DBC_MSSQL_MAX_ANY_NAME_LEN * 3) + 2) + 1];
	...
	*out_table_name    = '\0';
	...
	if ((in_name == NULL) || (!(*in_name)))
	...
	else if (strlen(in_name) > ((DBC_MSSQL_MAX_ANY_NAME_LEN * 3) + 2))
		sprintf(tmp_error_text,
			...
			strlen(in_name), (DBC_MSSQL_MAX_ANY_NAME_LEN * 3) + 2);
	else if ((point_count =
		...
		if (strlen(tmp_ptr[0]) > DBC_MSSQL_MAX_ANY_NAME_LEN)
			sprintf(tmp_error_text,
				"%s ('%s', length = %u) exceeds the maximum allowable (%u).",
				"The length of the database name component", tmp_ptr[0],
				strlen(tmp_ptr[0]), DBC_MSSQL_MAX_ANY_NAME_LEN);
		else if (strlen(tmp_ptr[1]) > DBC_MSSQL_MAX_ANY_NAME_LEN)
			sprintf(tmp_error_text,
				"%s ('%s', length = %u) exceeds the maximum allowable (%u).",
				"The length of the owner name component", tmp_ptr[1],
				strlen(tmp_ptr[1]), DBC_MSSQL_MAX_ANY_NAME_LEN);
		else if (strlen(tmp_ptr[2]) > DBC_MSSQL_MAX_ANY_NAME_LEN)
			sprintf(tmp_error_text,
				"%s ('%s', length = %u) exceeds the maximum allowable (%u).",
				"The length of the table name component", tmp_ptr[2],
				strlen(tmp_ptr[2]), DBC_MSSQL_MAX_ANY_NAME_LEN);
		else {
...
typedef struct {
	char source[(DBC_MSSQL_MAX_ANY_NAME_LEN * 3) + 1];
	char database_name[DBC_MSSQL_MAX_ANY_NAME_LEN + 1];
	char owner_name[DBC_MSSQL_MAX_ANY_NAME_LEN + 1];
	char table_name[DBC_MSSQL_MAX_ANY_NAME_LEN + 1];
} TEST_TEXT;
...
char **argv;
{
	...
	fprintf(stderr, "Test routine for 'DBC_MSSQL_BreakTableName()'\n");
	fprintf(stderr, "                 'DBC_MSSQL_BreakDatabaseName()'\n");
	fprintf(stderr, "---- ------- --- --------------------------------\n");
	...
	if (argc == 1) {
		for (count_1 = 0; count_1 < TEST_TextCount; count_1++) {
			...
			if (*test_ptr->source) {
				...
				if (DBC_MSSQL_BreakTableName(test_ptr->source, database_name,
			...
			if (*test_ptr->source) {
				*table_name = '\0';
				if (DBC_MSSQL_BreakDatabaseName(test_ptr->source, database_name,
		...
		for (count_1 = 2; count_1 < argc; count_1++) {
			printf("Source Text   : %s\n", argv[count_1]);
			if (DBC_MSSQL_BreakTableName(argv[count_1], database_name,
				owner_name, table_name, error_text) == DBC_SUCCESS)
				printf("Break Table   : [%s][%s][%s]\n",
					database_name, owner_name, table_name);
			else
				printf("ERROR Table   : %s\n", error_text);
			if (DBC_MSSQL_BreakDatabaseName(argv[count_1], database_name,
				owner_name, error_text) == DBC_SUCCESS)
				printf("Break Database: [%s][%s][%s]\n",
					database_name, owner_name, table_name);
			else
