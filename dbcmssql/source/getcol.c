...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetColumn(spec_string, in_table_name, column_count, column_list,
	error_text)
const char    *spec_string;
const char    *in_table_name;
unsigned int  *column_count;
DBC_MSSQL_COL  **column_list;
char          *error_text;
{
	int         return_code;
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetCols",
		...
		return_code = DBC_MSSQL_GetColumnBasic(dbc_context_data.dbc_control_ptr,
...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetColumnBasic(dbc_control_ptr, in_table_name, column_count,
...
char          *error_text;
{
	...
	DBC_MSSQL_DB_COL     *db_column_list  = NULL;
	...
	*column_list  = NULL;
	...
	if ((return_code = DBC_MSSQL_GetDBColumnBasic(dbc_control_ptr, in_table_name,
		&db_column_count, &db_column_list, error_text)) == DBC_SUCCESS) {
		if ((*column_list = ((DBC_MSSQL_COL *) calloc(db_column_count,
			sizeof(DBC_MSSQL_COL)))) == NULL) {
			STR_AllocMsgList(db_column_count, sizeof(DBC_MSSQL_COL),
			...
			*column_count = db_column_count;
			for (count_1 = 0; count_1 < *column_count; count_1++) {
				...
				(*column_list)[count_1].flag_bit_position =
					(db_column_list[count_1].type != SQLBIT) ? 0 :
				...
				if ((type_ptr =
					DBC_MSSQL_FIND_TypeByType((*column_list)[count_1].type,
					NULL)) != NULL) {
			...
			"%s ('%s.%s.%s') %s %u (named '%s') --- %s %d (named '%s') %s.",
						...
						db_column_list[count_1].name, "Microsoft SQL Server type id",
...
char **argv;
{
	...
	DBC_MSSQL_COL  *column_list  = NULL;
	unsigned int  count_1;
	char          error_text[DBC_MAX_ERROR_TEXT];

	STR_EMIT_CharLine('=', 78, NULL, NULL);

	fprintf(stderr, "Test routine for 'DBC_MSSQL_GetColumn()'\n");
	fprintf(stderr, "---- ------- --- ---------------------\n");
	...
	for (count_1 = 2; count_1 < argc; count_1++) {
		...
		if ((return_code = DBC_MSSQL_GetColumn(argv[1], argv[count_1],
			&column_count, &column_list, error_text)) != DBC_SUCCESS)
			break;
		fprintf(stderr, "%u columns in table.\n", column_count);
		DBC_MSSQL_EMIT_ColumnListIdx(column_count, column_list, NULL, NULL);
