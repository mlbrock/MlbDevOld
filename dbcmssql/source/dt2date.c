...
/*
	...
	File Description	:	Converts a DATFUNCS 'DTIME' structure to a Microsoft
								SQL Server date type.
...
/*	***********************************************************************	*/
int DBC_MSSQL_DTIMEToDate(dbc_type, dtime_ptr, date_ptr, error_text)
int          dbc_type;
...
char        *error_text;
{
	...
	switch (dbc_type) {
		case SQLDATETIM4	:
			DBC_MSSQL_DTIMEToMSSQL4(dtime_ptr, ((DBDATETIM4 *) date_ptr));
			break;
		case SQLDATETIME	:
			DBC_MSSQL_DTIMEToMSSQL8(dtime_ptr, ((DBDATETIME *) date_ptr));
			break;
		default				:
