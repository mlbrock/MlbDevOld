...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetIndex(spec_string, in_table_name, index_count, index_list,
	error_text)
const char    *spec_string;
const char    *in_table_name;
unsigned int  *index_count;
DBC_MSSQL_IDX  **index_list;
char          *error_text;
{
	int         return_code;
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetIdxs",
		...
		return_code = DBC_MSSQL_GetIndexBasic(dbc_context_data.dbc_control_ptr,
...
EOH */
/* *********************************************************************** */
int DBC_MSSQL_GetIndexBasic(dbc_control_ptr, in_table_name, index_count,
...
char          *error_text;
{
   ...
   DBC_MSSQL_COL    *column_list    = NULL;
	...
	DBC_MSSQL_DB_IDX *db_index_list  = NULL;
	DBC_MSSQL_DB_IDX *db_index_ptr;
	DBC_MSSQL_IDX     index_data;
	...
	*index_list  = NULL;
	...
	if ((return_code = DBC_MSSQL_GetColumnBasic(dbc_control_ptr, in_table_name,
		&column_count, &column_list, error_text)) == DBC_SUCCESS) {
		if (((return_code = DBC_MSSQL_GetDBIndexBasic(dbc_control_ptr,
			...
			for (count_1 = 0; count_1 < db_index_count; count_1++) {
				...
				if (index_data.column_count == DBC_MSSQL_MAX_IDX_COLS) {
...
char **argv;
{
	...
	DBC_MSSQL_IDX  *index_list  = NULL;
	unsigned int  count_1;
	char          error_text[DBC_MAX_ERROR_TEXT];

	STR_EMIT_CharLine('=', 78, NULL, NULL);

	fprintf(stderr, "Test routine for 'DBC_MSSQL_GetIndex()'\n");
	fprintf(stderr, "---- ------- --- --------------------\n");
	...
	for (count_1 = 2; count_1 < argc; count_1++) {
		...
		if ((return_code = DBC_MSSQL_GetIndex(argv[1], argv[count_1],
			&index_count, &index_list, error_text)) != DBC_SUCCESS)
			break;
		fprintf(stderr, "%u indices in table\n", index_count);
		DBC_MSSQL_EMIT_IndexListIdx(index_count, index_list, NULL, NULL);
