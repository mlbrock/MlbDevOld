...
/*
	...
	File Description	:	Converts a Microsoft SQL Server date type to a DATFUNCS
								'DTIME' structure date.
...
/*	***********************************************************************	*/
int DBC_MSSQL_DateToDTIME(dbc_type, date_ptr, dtime_ptr, error_text)
int         dbc_type;
const void *date_ptr;
DTIME      *dtime_ptr;
char       *error_text;
{
	...
	switch (dbc_type) {
		case SQLDATETIM4	:
			DBC_MSSQL_MSSQL4ToDTIME(((const DBDATETIM4 *) date_ptr), dtime_ptr);
			break;
		case SQLDATETIME	:
			DBC_MSSQL_MSSQL8ToDTIME(((const DBDATETIME *) date_ptr), dtime_ptr);
			break;
		default				:
