...
/* BOH
	...
	NAME			:	DBC_MSSQL_ParseConn

	SYNOPSIS		:	return_code = DBC_MSSQL_ParseConn(conn_type,
   ...
   COPYRIGHT   :	Copyright 1995 - 2015 Michael L. Brock
	...
	OUTPUT INDEX:	DBC_MSSQL_ParseConn
						DBC Microsoft SQL Server Functions:DBC_MSSQL_ParseConn
						DBC Functions:Connection Functions:DBC_MSSQL_ParseConn

	PUBLISH XREF:	DBC_MSSQL_ParseConn

	PUBLISH NAME:	DBC_MSSQL_ParseConn

	ENTRY CLASS	:	DBC Functions:Connection Functions

EOH */
/* *********************************************************************** */
int DBC_MSSQL_ParseConn(conn_type, conn_string, parsed_count,
	...
	if ((arg_data_ptr = strdup(conn_string)) == NULL) {
		...
		if (sepwordx_keep(trim(noblank(arg_data_ptr)), ",", &tmp_count,
			...
			for (count_1 = 0; count_1 < tmp_count; count_1++) {
				if (strlen(tmp_list[count_1]) > DBC_MSSQL_MAX_ANY_NAME_LEN) {
