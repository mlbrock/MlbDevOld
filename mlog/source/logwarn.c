...
/*
	...
	File Description	:	Logs messages with priority 'LOG_WARNING'.
...
/*VARARGS2*/
#ifndef NO_STDARGS
int MLOG_LogWarn(MLOG *mlog_ptr, const char *in_format, ...)
#else
int MLOG_LogWarn(mlog_ptr, in_format, va_list)
MLOG       *mlog_ptr;
...
#endif /* #ifndef NO_STDARGS */
{
	...
	return_code = MLOG_LogWarnVA(mlog_ptr, in_format, arg_list);
...
/*	***********************************************************************	*/
#ifndef NARGS
int MLOG_LogWarnVA(MLOG *mlog_ptr, const char *in_format, va_list arg_list)
#else
int MLOG_LogWarnVA(mlog_ptr, in_format, arg_list)
MLOG       *mlog_ptr;
const char *in_format;
va_list     arg_list;
#endif /* #ifndef NARGS */
{
	return((!MLOG_DO_LOG(mlog_ptr)) ? MLOG_FAILURE :
		MLOG_LogBasicVA(mlog_ptr, NULL, MLOG_PRIORITY_WARNING,
...
/*VARARGS2*/
#ifndef NO_STDARGS
int MLOG_LogWarnTime(MLOG *mlog_ptr, const struct timeval *in_time,
	const char *in_format, ...)
#else
int MLOG_LogWarnTime(mlog_ptr, in_time, in_format, va_list)
MLOG                 *mlog_ptr;
...
#endif /* #ifndef NO_STDARGS */
{
	...
	return_code = MLOG_LogWarnTimeVA(mlog_ptr, in_time, in_format, arg_list);
...
/*	***********************************************************************	*/
#ifndef NARGS
int MLOG_LogWarnTimeVA(MLOG *mlog_ptr, const struct timeval *in_time,
	const char *in_format, va_list arg_list)
#else
int MLOG_LogWarnTimeVA(mlog_ptr, in_time, in_format, arg_list)
MLOG                 *mlog_ptr;
const struct timeval *in_time;
const char           *in_format;
va_list               arg_list;
...
{
	return((!MLOG_DO_LOG(mlog_ptr)) ? MLOG_FAILURE :
		MLOG_LogBasicVA(mlog_ptr, in_time, MLOG_PRIORITY_WARNING,
		mlog_ptr->user_string, mlog_ptr->flags, mlog_ptr->mfile_mask,
		mlog_ptr->file_mask, mlog_ptr->syslog_mask, in_format, arg_list));
}
/*	***********************************************************************	*/
...
#define TEST_MLOG_FILE_NAME		"TEST_MLOG_LogWarn.txt"
...
int main()
{
	...
	fprintf(stderr, "%s\n%s\n\n",
		"Test harness for function 'MLOG_LogWarn()'\n",
		"                          'MLOG_LogWarnVA()'\n");
	fprintf(stderr,
		"---- ------- --- -------- ------------------\n\n");
	...
	if ((mlog_ptr = MLOG_FOpenDefault(TEST_MLOG_FILE_NAME)) != NULL) {
		...
		for (count_1 = 0; count_1 < 26; count_1++) {
			MLOG_LogWarn(mlog_ptr, "Try number %10u: %s\n",
