...
/*
	...
	File Description	:	Logs messages with priority 'LOG_ERR'.
...
/*VARARGS2*/
#ifndef NO_STDARGS
int MLOG_LogErr(MLOG *mlog_ptr, const char *in_format, ...)
#else
int MLOG_LogErr(mlog_ptr, in_format, va_list)
MLOG       *mlog_ptr;
...
#endif /* #ifndef NO_STDARGS */
{
	...
	return_code = MLOG_LogErrVA(mlog_ptr, in_format, arg_list);
...
/*	***********************************************************************	*/
#ifndef NARGS
int MLOG_LogErrVA(MLOG *mlog_ptr, const char *in_format, va_list arg_list)
#else
int MLOG_LogErrVA(mlog_ptr, in_format, arg_list)
MLOG       *mlog_ptr;
const char *in_format;
va_list     arg_list;
#endif /* #ifndef NARGS */
{
	return((!MLOG_DO_LOG(mlog_ptr)) ? MLOG_FAILURE :
		MLOG_LogBasicVA(mlog_ptr, NULL, MLOG_PRIORITY_ERR,
...
/*VARARGS2*/
#ifndef NO_STDARGS
int MLOG_LogErrTime(MLOG *mlog_ptr, const struct timeval *in_time,
	const char *in_format, ...)
#else
int MLOG_LogErrTime(mlog_ptr, in_time, in_format, va_list)
MLOG                 *mlog_ptr;
...
#endif /* #ifndef NO_STDARGS */
{
	...
	return_code = MLOG_LogErrTimeVA(mlog_ptr, in_time, in_format, arg_list);
...
/*	***********************************************************************	*/
#ifndef NARGS
int MLOG_LogErrTimeVA(MLOG *mlog_ptr, const struct timeval *in_time,
	const char *in_format, va_list arg_list)
#else
int MLOG_LogErrTimeVA(mlog_ptr, in_time, in_format, arg_list)
MLOG                 *mlog_ptr;
const struct timeval *in_time;
const char           *in_format;
va_list               arg_list;
...
{
	return((!MLOG_DO_LOG(mlog_ptr)) ? MLOG_FAILURE :
		MLOG_LogBasicVA(mlog_ptr, in_time, MLOG_PRIORITY_ERR,
		mlog_ptr->user_string, mlog_ptr->flags, mlog_ptr->mfile_mask,
		mlog_ptr->file_mask, mlog_ptr->syslog_mask, in_format, arg_list));
}
/*	***********************************************************************	*/
...
int main()
{
	...
	fprintf(stderr, "%s\n%s\n\n",
		"Test harness for function 'MLOG_LogErr()'\n",
		"                          'MLOG_LogErrVA()'\n");
	fprintf(stderr,
		"---- ------- --- -------- -----------------\n\n");
	...
	if ((mlog_ptr = MLOG_FOpenDefault(TEST_MLOG_FILE_NAME)) != NULL) {
		...
		for (count_1 = 0; count_1 < 26; count_1++) {
			MLOG_LogErr(mlog_ptr, "Try number %10u: %s\n",
