...
/*
	...
	File Description	:	Gets the next record in an SIIDR structure.
...
/*	***********************************************************************	*/
int SIIDR_GetNext(SIIDR *siidr_ptr, const void *in_record_ptr,
	...
	SIIDR_GetSpecFunc get_spec_func)
{
	...
	SIIDR_GetNextBasic(siidr_ptr, in_record_ptr, out_index, out_record_ptr,
		user_data_ptr, get_spec_func, &found_flag, error_text);
...
/*	***********************************************************************	*/
int SIIDR_GetNextBasic(SIIDR *siidr_ptr, const void *in_record_ptr,
	unsigned int *out_index, void *out_record_ptr, void *user_data_ptr,
	SIIDR_GetSpecFunc get_spec_func, int *found_flag, char *error_text)
{
	...
	if (((return_code = SIIDR_ClearErrorOpt(error_text)) == SIIDR_SUCCESS) &&
		...
		if ((return_code = SIIDR_LockReaderIDR(siidr_ptr, error_text)) ==
			...
			*found_flag = SIIDR_GetNextRaw(siidr_ptr, in_record_ptr,
...
/*	***********************************************************************	*/
int SIIDR_GetNextRaw(SIIDR *siidr_ptr, const void *in_record_ptr,
	unsigned int *out_index, void *out_record_ptr, void *user_data_ptr,
	SIIDR_GetSpecFunc get_spec_func)
{
	int          found_flag = SIIDR_FALSE;
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr,
		"Test harness for function 'SIIDR_GetNext()'\n");
	fprintf(stderr,
		"                          'SIIDR_GetNextRaw()'\n");
	fprintf(stderr,
		"                          'SIIDR_GetNextBasic()'\n");
	...
	if ((return_code = SIIDR_GetNextBasic(siidr_ptr, NULL, &out_index,
	...
	if ((return_code = SIIDR_GetNextBasic(siidr_ptr, NULL, &out_index,
		...
		goto EXIT_FUNCTION;
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text, "!!!Next record not found in non-empty IDR!!!");
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Next record in IDR:\n");
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);

	in_record = out_record;
	if ((return_code = SIIDR_GetNextBasic(siidr_ptr, &in_record,
	...
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text,
			"!!!Next record with odd quantity not found in non-empty IDR!!!");
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Next record with odd quantity in IDR:\n");
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);
	...
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text,
			"!!!Next record with quantity = %u not found in non-empty IDR!!!",
			tmp_quantity);
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Next record with quantity = %u in IDR:\n",
		tmp_quantity);
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);
