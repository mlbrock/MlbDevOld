...
/*
	...
	File Description	:	Gets the last record in an SIIDR structure.
...
/*	***********************************************************************	*/
int SIIDR_GetLast(SIIDR *siidr_ptr, unsigned int *out_index,
	...
	SIIDR_GetSpecFunc get_spec_func)
{
	...
	SIIDR_GetLastBasic(siidr_ptr, out_index, out_record_ptr, user_data_ptr,
...
/*	***********************************************************************	*/
int SIIDR_GetLastBasic(SIIDR *siidr_ptr, unsigned int *out_index,
	void *out_record_ptr, void *user_data_ptr,
	SIIDR_GetSpecFunc get_spec_func, int *found_flag, char *error_text)
{
	...
	if (((return_code = SIIDR_ClearErrorOpt(error_text)) == SIIDR_SUCCESS) &&
		...
		if ((return_code = SIIDR_LockReaderIDR(siidr_ptr, error_text)) ==
			...
			*found_flag = SIIDR_GetLastRaw(siidr_ptr, out_index,
...
/*	***********************************************************************	*/
int SIIDR_GetLastRaw(SIIDR *siidr_ptr, unsigned int *out_index,
	void *out_record_ptr, void *user_data_ptr,
	SIIDR_GetSpecFunc get_spec_func)
{
	int          found_flag = SIIDR_FALSE;
	...
	if (siidr_ptr->record_count) {
		found_index = siidr_ptr->record_count - 1;
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr,
		"Test harness for function 'SIIDR_GetLast()'\n");
	fprintf(stderr,
		"                          'SIIDR_GetLastRaw()'\n");
	fprintf(stderr,
		"                          'SIIDR_GetLastBasic()'\n");
	...
	if ((return_code = SIIDR_GetLastBasic(siidr_ptr, &out_index,
	...
	if ((return_code = SIIDR_GetLastBasic(siidr_ptr, &out_index,
		...
		goto EXIT_FUNCTION;
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text, "!!!Last record not found in non-empty IDR!!!");
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Last record in IDR:\n");
	...
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text,
			"!!!Last record with odd quantity not found in non-empty IDR!!!");
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Last record with odd quantity in IDR:\n");
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);
	...
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text,
			"!!!Last record with quantity = %u not found in non-empty IDR!!!",
			tmp_quantity);
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Last record with quantity = %u in IDR:\n",
		tmp_quantity);
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);
