...
/*
	...
	File Description	:	Gets the previous record in an SIIDR structure.
...
/*	***********************************************************************	*/
int SIIDR_GetPrevious(SIIDR *siidr_ptr, const void *in_record_ptr,
	...
	SIIDR_GetSpecFunc get_spec_func)
{
	...
	SIIDR_GetPreviousBasic(siidr_ptr, in_record_ptr, out_index,
		out_record_ptr, user_data_ptr, get_spec_func, &found_flag, error_text);
...
/*	***********************************************************************	*/
int SIIDR_GetPreviousBasic(SIIDR *siidr_ptr, const void *in_record_ptr,
	unsigned int *out_index, void *out_record_ptr, void *user_data_ptr,
	SIIDR_GetSpecFunc get_spec_func, int *found_flag, char *error_text)
{
	...
	if (((return_code = SIIDR_ClearErrorOpt(error_text)) == SIIDR_SUCCESS) &&
		...
		if ((return_code = SIIDR_LockReaderIDR(siidr_ptr, error_text)) ==
			...
			*found_flag = SIIDR_GetPreviousRaw(siidr_ptr, in_record_ptr,
				out_index, out_record_ptr, user_data_ptr, get_spec_func);
...
/*	***********************************************************************	*/
int SIIDR_GetPreviousRaw(SIIDR *siidr_ptr, const void *in_record_ptr,
	unsigned int *out_index, void *out_record_ptr, void *user_data_ptr,
	SIIDR_GetSpecFunc get_spec_func)
{
	int          found_flag = SIIDR_FALSE;
	...
	if (in_record_ptr == NULL)
		found_flag = SIIDR_GetLastRaw(siidr_ptr, out_index, out_record_ptr,
			user_data_ptr, get_spec_func);
	else if (siidr_ptr->record_count) {
		if (SIIDR_FindRecordRaw(siidr_ptr, in_record_ptr, &found_index) !=
			NULL)
			found_index--;
		if (found_index < siidr_ptr->record_count) {
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr,
		"Test harness for function 'SIIDR_GetPrevious()'\n");
	fprintf(stderr,
		"                          'SIIDR_GetPreviousRaw()'\n");
	fprintf(stderr,
		"                          'SIIDR_GetPreviousBasic()'\n");
	fprintf(stderr,
		"---- ------- --- -------- --------------------------\n\n");
	...
	if ((return_code = SIIDR_GetPreviousBasic(siidr_ptr, NULL, &out_index,
	...
	if ((return_code = SIIDR_GetPreviousBasic(siidr_ptr, NULL, &out_index,
		...
		goto EXIT_FUNCTION;
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text, "!!!Previous record not found in non-empty IDR!!!");
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Previous record in IDR:\n");
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);

	in_record = out_record;
	if ((return_code = SIIDR_GetPreviousBasic(siidr_ptr, &in_record,
		&out_index, &out_record, NULL, NULL, &found_flag, error_text)) !=
		SIIDR_SUCCESS)
		goto EXIT_FUNCTION;
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text, "!!!Previous record not found in non-empty IDR!!!");
	...
	in_record = out_record;
	if ((return_code = SIIDR_GetPreviousBasic(siidr_ptr, &in_record,
		&out_index, &out_record, NULL, 
		((SIIDR_GetSpecFunc) SIIDR_TEST_GetSpecQuantityOdd),
		&found_flag, error_text)) != SIIDR_SUCCESS)
		goto EXIT_FUNCTION;
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text,
			"!!!Previous record with odd quantity not found in non-empty IDR!!!");
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Previous record with odd quantity in IDR:\n");
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);
	...
	if ((return_code = SIIDR_GetPreviousBasic(siidr_ptr, &in_record,
		&out_index, &out_record, &tmp_quantity,
		((SIIDR_GetSpecFunc) SIIDR_TEST_GetSpecQuantity), &found_flag,
		error_text)) != SIIDR_SUCCESS)
		goto EXIT_FUNCTION;
	if (found_flag != SIIDR_TRUE) {
		sprintf(error_text,
			"!!!Previous record with quantity = %u not found in non-empty IDR!!!",
			tmp_quantity);
		return_code = SIIDR_FAILURE;
		goto EXIT_FUNCTION;
	}
	printf("********** Previous record with quantity = %u in IDR:\n",
		tmp_quantity);
	SIIDR_TEST_EmitRecord(out_index, &out_record);
	STR_EMIT_CharLine('=', 77, NULL, NULL);
