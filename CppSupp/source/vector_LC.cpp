...
/*
	...
	File Description	:	Test harness module for template class vector_LC<>.
...
/*	***********************************************************************	*/
typedef std::vector<int>                                        BaseCollType;
typedef MLB::CPPSupp::STLSupp::vector_LC<int>                   NullCollType;
typedef MLB::CPPSupp::STLSupp::vector_LC<int, MLB::MTIF::MtrO>  ThisCollType;
...
static bool TEST_Cons_BasicCollType(const char *func_name, unsigned int element_count);
...
static bool TEST_Capacity(const char *func_name, unsigned int element_count);
...
static bool TEST_Reserve(const char *func_name, unsigned int element_count);
...
static const TEST_CONTROL TEST_FuncList[] = {
	...
	TEST_MAKE_FUNC_ENTRY(TEST_Cons_BasicCollType),
	...
	TEST_MAKE_FUNC_ENTRY(TEST_Capacity),
	...
	TEST_MAKE_FUNC_ENTRY(TEST_Reserve),
...
int main(int argc, char **argv)
{
	...
	std::cerr << "Test harness for MLB::CPPSupp::STLSupp::vector_LC<>" <<
		std::endl << std::endl;
	...
	TEST_SUPP_EmitBarEqual();
	std::cout << "Test harness for MLB::CPPSupp::STLSupp::vector_LC<>" <<
		std::endl;
	TEST_SUPP_EmitBarEqual();
	...
	try {
		{
			...
			TEST_SUPP_EmitResults(func_name,
				"vector_LC::MyLockReader()", ok_flag,
				true,
		...
		reader_test = false;
		{
			...
			TEST_SUPP_EmitResults(func_name,
				"vector_LC::MyLockWriter()", ok_flag,
				true,
		...
		TEST_SUPP_EmitResults(func_name,
			((reader_test) ? "vector_LC::MyLockReader()" : "vector_LC::MyLockWriter()"),
	...
	unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::PopBackWithGet(*for-each-element*)",
		ok_flag,
		ok_flag,
...
static bool TEST_Cons_1_A(const char *func_name, unsigned int element_count)
{
	...
	if ((ok_flag = TEST_SUPP_EmitResults(func_name, "vector_LC::vector_LC()",
		ok_flag,
		(this_coll == base_coll),
		...
		ok_flag = TEST_SUPP_EmitResults(func_name,
			"vector_LC::vector_LC() --- with data", ok_flag,
			(this_coll == base_coll),
...
static bool TEST_Cons_1_B(const char *func_name, unsigned int element_count)
{
	...
	if ((ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::vector_LC(allocator<int>())", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)")) == true) {
		TEST_SUPP_FillCollThis(0, element_count, this_coll);
		TEST_SUPP_FillCollBase(0, element_count, base_coll);
		ok_flag = TEST_SUPP_EmitResults(func_name,
			"vector_LC::vector_LC(allocator<int>()) --- with data", ok_flag,
			(this_coll == base_coll),
			"(this_coll == base_coll)");
	}
...
static bool TEST_Cons_2_A(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::vector_LC(element_count)", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Cons_2_B(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::vector_LC(element_count, 666)", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
	...
	unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::vector_LC(vector<>::begin(), vector<>::end())", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
/*	***********************************************************************	*/
static bool TEST_Cons_BasicCollType(const char *func_name,
	unsigned int element_count)
{
	bool         ok_flag = true;
	BaseCollType base_coll(element_count, 666);
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::vector_LC(const vector<> &)", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_ConsCopy(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::vector_LC(const vector_LC<> &)", ok_flag,
		(this_coll == copy_coll),
		"(this_coll == copy_coll)");
...
static bool TEST_Assign_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Assign(vector<>::iterator first, vector<>::iterator last)",
		ok_flag,
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
"vector_LC::Assign(vector_LC<>::iterator first, vector<>_LC::iterator last)",
		ok_flag,
...
static bool TEST_Assign_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Assign(vector<>::size_type count, vector<>::value_type data)",
		ok_flag,
		(this_coll_1 == base_coll),
		"(this_coll_1 == base_coll)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
"vector_LC::Assign(vector_LC<>::size_type count, vector_LC<>::value_type data)",
		ok_flag,
		(this_coll_1 == this_coll_2),
		"(this_coll_1 == this_coll_2)");
...
static bool TEST_At(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::At(vector_LC<>::size_type in_index)",
		ok_flag,
		ok_flag,
...
static bool TEST_Capacity(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Capacity() --- before adding elements",
		ok_flag,
		(this_coll.Capacity() >= 0),
		"(this_coll.Capacity() >= 0)");

	TEST_SUPP_FillCollThis(0, element_count, this_coll);

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Capacity() --- after adding elements",
		ok_flag,
		(this_coll.Capacity() >= element_count),
		"(this_coll.Capacity() >= element_count)");
...
static bool TEST_Empty(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Empty() --- before vector_LC::Clear()",
		ok_flag,
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Empty() --- after vector_LC::Clear()",
		ok_flag,
...
static bool TEST_Erase_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Erase(Iter *middle-of-container*)",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Erase_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Erase(vector_LC::Begin(), vector_LC::End())",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Insert_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Insert(Iter position = vector_LC::Begin(), -1)",
		ok_flag,
		(this_coll == base_coll),
		"(vector_LC<> == vector<>)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Insert(Iter position = vector_LC::End(), -2)",
		ok_flag,
		(this_coll == base_coll),
		"(vector_LC<> == vector<>)");
...
static bool TEST_Insert_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Insert(Iter position = vector_LC::Begin(), element_count, -1)",
		ok_flag,
		(this_coll == base_coll),
		"(vector_LC<> == vector<>)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Insert(Iter position = vector_LC::End(), element_count, -2)",
		ok_flag,
		(this_coll == base_coll),
		"(vector_LC<> == vector<>)");
...
ThisCollType src_coll_1(element_count, -1);
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Insert(Iter position = vector_LC::Begin(), \
vector<>::begin(), vector<>::end())",
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Insert(Iter position = vector_LC::End(), \
vector<>::begin(), vector<>::end())",
		ok_flag,
		(this_coll == base_coll),
		"(vector_LC<> == vector<>)");
...
{
	bool         ok_flag = true;
	ThisCollType this_coll(element_count, 123);
	BaseCollType base_coll(element_count, 123);

	this_coll.Reserve(this_coll.Capacity() + 1);
	base_coll.reserve(base_coll.capacity() + 1);

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Reserve(vector_LC::Capacity() + 1)",
		ok_flag,
		(this_coll.Capacity() == base_coll.capacity()),
		"(this_coll.Capacity() == base_coll.capacity())");
...
static bool TEST_Resize(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Size(vector_LC::size() * 2, 456)",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Size(vector_LC::size() / 4, 789)",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Swap(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Swap(vector<>)",
		ok_flag,
		(this_coll_1 == base_coll_1),
		"(this_coll_1 == base_coll_1)");

	//	Just tests to ensure the implementation is compilable and functional...
	ThisCollType this_coll_4A(element_count, 123);
	ThisCollType this_coll_4B(element_count, 123);
	MLB::CPPSupp::STLSupp::swap(this_coll_4A, this_coll_4B);
//	swap(this_coll_4A, this_coll_4B);
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC::Swap(vector_LC<>, vector_LC<>)",
		ok_flag,
		(this_coll_4A == this_coll_4B),
		"(this_coll_4A == this_coll_4B");
...
static bool TEST_Op_Eq(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC<element_count, 12345> == vector_LC<element_count, 12345>",
		ok_flag,
		(this_coll_1 == this_coll_2),
		"(vector_LC<> == vector_LC<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"!(vector_LC<element_count, 12345> == vector_LC<element_count, 54321>)",
		ok_flag,
		!(this_coll_1 == this_coll_3),
		"!(vector_LC<> == vector_LC<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"!(vector_LC<element_count, 12345> == vector_LC<element_count + 1, 12345>)",
		ok_flag,
		!(this_coll_1 == this_coll_4),
		"!(vector_LC<> == vector_LC<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC<element_count, 12345> == vector<element_count, 12345>",
		ok_flag,
		(this_coll_1 == base_coll),
		"(vector_LC<> == vector<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector<element_count, 12345> == vector_LC<element_count, 12345>",
		ok_flag,
...
static bool TEST_GetData_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC<> vector_LC::GetData()", ok_flag,
...
static bool TEST_GetData_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"void vector_LC::GetData(vector_LC<> &)", ok_flag,
...
static bool TEST_GetDataRef(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC<> &vector_LC::GetDataRef()", ok_flag,
		(this_coll.GetDataRef() == base_coll),
		"this_coll.GetDataRef() == base_coll");
...
static bool TEST_GetDataPtr(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"vector_LC<> *vector_LC::GetDataPtr()", ok_flag,
