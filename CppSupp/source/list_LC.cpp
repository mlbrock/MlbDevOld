...
/*
	...
	File Description	:	Test harness module for template class list_LC<>.
...
/*	***********************************************************************	*/
typedef std::list<int>                                        BaseCollType;
typedef MLB::CPPSupp::STLSupp::list_LC<int>                   NullCollType;
typedef MLB::CPPSupp::STLSupp::list_LC<int, MLB::MTIF::MtrO>  ThisCollType;
...
static bool TEST_Merge(const char *func_name, unsigned int element_count);
static bool TEST_MergeTraits(const char *func_name, unsigned int element_count);
...
static bool TEST_Remove(const char *func_name, unsigned int element_count);
static bool TEST_Remove_If(const char *func_name, unsigned int element_count);
...
static bool TEST_Reverse(const char *func_name, unsigned int element_count);
static bool TEST_Splice(const char *func_name, unsigned int element_count);
static bool TEST_Swap(const char *func_name, unsigned int element_count);
static bool TEST_Unique(const char *func_name, unsigned int element_count);
...
static const TEST_CONTROL TEST_FuncList[] = {
	...
	TEST_MAKE_FUNC_ENTRY(TEST_Merge),
	TEST_MAKE_FUNC_ENTRY(TEST_MergeTraits),
	...
	TEST_MAKE_FUNC_ENTRY(TEST_Remove),
	TEST_MAKE_FUNC_ENTRY(TEST_Remove_If),
	TEST_MAKE_FUNC_ENTRY(TEST_Rend),
	TEST_MAKE_FUNC_ENTRY(TEST_Resize),
	TEST_MAKE_FUNC_ENTRY(TEST_Reverse),
	TEST_MAKE_FUNC_ENTRY(TEST_Splice),
	TEST_MAKE_FUNC_ENTRY(TEST_Swap),
	TEST_MAKE_FUNC_ENTRY(TEST_Unique),
...
int main(int argc, char **argv)
{
	...
	std::cerr << "Test harness for MLB::CPPSupp::STLSupp::list_LC<>" <<
		std::endl << std::endl;
	...
	TEST_SUPP_EmitBarEqual();
	std::cout << "Test harness for MLB::CPPSupp::STLSupp::list_LC<>" <<
		std::endl;
	TEST_SUPP_EmitBarEqual();
...
static bool TEST_ScopedLocks(const char *func_name, unsigned int element_count)
{
	...
	try {
		...
		TEST_SUPP_EmitResults(func_name,
			((reader_test) ? "list_LC::MyLockReader()" : "list_LC::MyLockWriter()"),
	...
	unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::PopBackWithGet(*for-each-element*)",
		ok_flag,
		ok_flag,
	...
	unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::PopFrontWithGet(*for-each-element*)",
		ok_flag,
		ok_flag,
...
static bool TEST_Cons_1_A(const char *func_name, unsigned int element_count)
{
	...
	if ((ok_flag = TEST_SUPP_EmitResults(func_name, "list_LC::list_LC()",
		ok_flag,
		(this_coll == base_coll),
		...
		ok_flag = TEST_SUPP_EmitResults(func_name,
			"list_LC::list_LC() --- with data", ok_flag,
			(this_coll == base_coll),
...
static bool TEST_Cons_1_B(const char *func_name, unsigned int element_count)
{
	...
	if ((ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::list_LC(allocator<int>())", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)")) == true) {
		TEST_SUPP_FillCollThis(0, element_count, this_coll);
		TEST_SUPP_FillCollBase(0, element_count, base_coll);
		ok_flag = TEST_SUPP_EmitResults(func_name,
			"list_LC::list_LC(allocator<int>()) --- with data", ok_flag,
			(this_coll == base_coll),
			"(this_coll == base_coll)");
	}
...
static bool TEST_Cons_2_A(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::list_LC(element_count)", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Cons_2_B(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::list_LC(element_count, 666)", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
	...
	unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::list_LC(list<>::begin(), list<>::end())", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
	...
	unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::list_LC(const list<> &)", ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_ConsCopy(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::list_LC(const list_LC<> &)", ok_flag,
		(this_coll == copy_coll),
		"(this_coll == copy_coll)");
...
static bool TEST_Assign_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Assign(list<>::iterator first, list<>::iterator last)",
		ok_flag,
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
"list_LC::Assign(list_LC<>::iterator first, list<>_LC::iterator last)",
		ok_flag,
...
static bool TEST_Assign_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Assign(list<>::size_type count, list<>::value_type data)",
		ok_flag,
		(this_coll_1 == base_coll),
		"(this_coll_1 == base_coll)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
"list_LC::Assign(list_LC<>::size_type count, list_LC<>::value_type data)",
		ok_flag,
		(this_coll_1 == this_coll_2),
		"(this_coll_1 == this_coll_2)");
...
static bool TEST_Empty(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Empty() --- before list_LC::Clear()",
		ok_flag,
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Empty() --- after list_LC::Clear()",
		ok_flag,
...
static bool TEST_Erase_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Erase(Iter *middle-of-container*)",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Erase_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Erase(list_LC::Begin(), list_LC::End())",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Insert_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Insert(Iter position = list_LC::Begin(), -1)",
		ok_flag,
		(this_coll == base_coll),
		"(list_LC<> == list<>)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Insert(Iter position = list_LC::End(), -2)",
		ok_flag,
		(this_coll == base_coll),
		"(list_LC<> == list<>)");
...
static bool TEST_Insert_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Insert(Iter position = list_LC::Begin(), element_count, -1)",
		ok_flag,
		(this_coll == base_coll),
		"(list_LC<> == list<>)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Insert(Iter position = list_LC::End(), element_count, -2)",
		ok_flag,
		(this_coll == base_coll),
		"(list_LC<> == list<>)");
...
static bool TEST_Insert_3(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Insert(Iter position = list_LC::Begin(), \
list<>::begin(), list<>::end())",
		ok_flag,
		(this_coll == base_coll),
		"(list_LC<> == list<>)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Insert(Iter position = list_LC::End(), \
list<>::begin(), list<>::end())",
		ok_flag,
		(this_coll == base_coll),
		"(list_LC<> == list<>)");
...
static bool TEST_Merge(const char *func_name, unsigned int element_count)
{
	...
	ThisCollType this_coll;
	BaseCollType base_coll;
	ThisCollType this_src;
	BaseCollType base_src;

	TEST_SUPP_FillCollThis(0, element_count, this_coll);
	TEST_SUPP_FillCollBase(0, element_count, base_coll);
	TEST_SUPP_FillCollThis(element_count + 1, 10, this_coll);
	TEST_SUPP_FillCollBase(element_count + 1, 10, base_coll);

	this_coll.Merge(this_src);
	base_coll.merge(base_src);
...
static bool TEST_MergeTraits(const char *func_name, unsigned int element_count)
{
	...
	this_coll.Sort(std::greater<int>());
	base_coll.sort(std::greater<int>());

	this_coll.Merge(this_src, std::greater<int>());
	base_coll.merge(base_src, std::greater<int>());
...
/*
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Remove()",
		ok_flag,
		(*this_coll.Remove() == ((int) (element_count - 1))),
		"(*this_coll.Remove() == *expected-value*)");
...
/*
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Remove_If()",
		ok_flag,
		(*this_coll.Remove_If() == ((int) (element_count - 1))),
		"(*this_coll.Remove_If() == *expected-value*)");
...
static bool TEST_Resize(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Size(list_LC::size() * 2, 456)",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC::Size(list_LC::size() / 4, 789)",
		ok_flag,
		(this_coll == base_coll),
		"(this_coll == base_coll)");
...
static bool TEST_Op_Eq(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC<element_count, 12345> == list_LC<element_count, 12345>",
		ok_flag,
		(this_coll_1 == this_coll_2),
		"(list_LC<> == list_LC<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"!(list_LC<element_count, 12345> == list_LC<element_count, 54321>)",
		ok_flag,
		!(this_coll_1 == this_coll_3),
		"!(list_LC<> == list_LC<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"!(list_LC<element_count, 12345> == list_LC<element_count + 1, 12345>)",
		ok_flag,
		!(this_coll_1 == this_coll_4),
		"!(list_LC<> == list_LC<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC<element_count, 12345> == list<element_count, 12345>",
		ok_flag,
		(this_coll_1 == base_coll),
		"(list_LC<> == list<>)");

	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list<element_count, 12345> == list_LC<element_count, 12345>",
		ok_flag,
...
static bool TEST_GetData_1(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC<> list_LC::GetData()", ok_flag,
...
static bool TEST_GetData_2(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"void list_LC::GetData(list_LC<> &)", ok_flag,
...
static bool TEST_GetDataRef(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC<> &list_LC::GetDataRef()", ok_flag,
		(this_coll.GetDataRef() == base_coll),
		"this_coll.GetDataRef() == base_coll");
...
static bool TEST_GetDataPtr(const char *func_name, unsigned int element_count)
{
	...
	ok_flag = TEST_SUPP_EmitResults(func_name,
		"list_LC<> *list_LC::GetDataPtr()", ok_flag,
