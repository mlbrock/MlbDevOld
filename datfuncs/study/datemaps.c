...
/*
	...
	File Description	:	Low-level support for conversion of JULIAN dates to
								and from year/month/day dates.

	Revision History	:	1998-02-23 --- Creation.
									Michael L. Brock
...
/* *********************************************************************** */
/* *********************************************************************** */
/*		Function protototypes for the low-level functions which support the	*/
/*	conversion of JULIAN dates to and from year/month/day dates.				*/
/* *********************************************************************** */
COMPAT_FN_DECL(int    jultoymd_basic, (JULIAN in_julian, int *out_year_ptr,
	int *out_month_ptr, int *out_day_ptr, unsigned int year_map_count,
	const DATFUNCS_DATE_MAP *year_map_list));
COMPAT_FN_DECL(JULIAN ymdtojul_basic, (int in_year, int in_month, int in_day,
	unsigned int year_map_count, const DATFUNCS_DATE_MAP *year_map_list));
COMPAT_FN_DECL(int    DATFUNCS_CreateDateMap, (unsigned int first_year,
	unsigned int last_year, int century_flag, unsigned int *date_map_count,
...
/* *********************************************************************** */
COMPAT_FN_DECL_STATIC(int                      DATFUNCS_CMP_DateMap,
	(void *control_ptr, const DATFUNCS_DATE_MAP *ptr_1,
	const DATFUNCS_DATE_MAP *ptr_2, size_t data_length));
COMPAT_FN_DECL_STATIC(const DATFUNCS_DATE_MAP *DATFUNCS_FIND_DateMap,
...
#endif /* #ifndef NARGS */
{
	...
	if (ymdok(in_year, in_month, in_day) == DATFUNCS_TRUE) {
		...
		year_map_list     = ((!year_map_count) || (year_map_list == NULL)) ?
			DATFUNCS_YearMapList  : year_map_list;
		if ((((unsigned int) in_year) < year_map_list[0].year) ||
			(((unsigned int) in_year) > year_map_list[year_map_count - 1].year)) {
			tmp_julian =
...
COMPAT_FN_DECL_STATIC(int    DoCreate, (void));
COMPAT_FN_DECL_STATIC(void   DoCreateEmit, (unsigned int date_map_count,
	const DATFUNCS_DATE_MAP *date_map_list, const char *name_tag));
COMPAT_FN_DECL_STATIC(int    OLD_jultoymd, (JULIAN in_julian,
	int *out_year_ptr, int *out_month_ptr, int *out_day_ptr));
COMPAT_FN_DECL_STATIC(JULIAN OLD_ymdtojul, (int in_year, int in_month,
	int in_day));
COMPAT_FN_DECL_STATIC(int DoSpeedTest, (int first_year, int last_year));
COMPAT_FN_DECL_STATIC(int DoRegressionTest, (int first_year, int last_year));
...
*/
	...
	if ((return_code = DoSpeedTest(first_year, last_year)) ==
		DATFUNCS_SUCCESS)
		return_code = DoRegressionTest(first_year, last_year);
...
/*	*********************************************************************** */
#ifndef NARGS
static int DoSpeedTest(int first_year, int last_year)
#else
static int DoSpeedTest(first_year, last_year)
int first_year;
int last_year;
#endif /* #ifndef NARGS */
{
	...
	printf("Old algorithm: timing years from %04u to %04u ",
		first_year, last_year);
	previous_year = first_year;
	SDTIF_GetTimeNSecs(&start_time);
	for (count_1 = first_year; count_1 <= last_year; count_1++) {
		for (count_2 = first_month; count_2 <= last_month; count_2++) {
			max_dom = mndaymax(count_1, count_2);
			for (count_3 = first_day; count_3 <= max_dom; count_3++) {
	...
	SDTIF_GetTimeNSecs(&end_time);
	total_time_1 = (SDTIF_GET_TIMESPEC_NSECS_DOUBLE(&end_time) -
		SDTIF_GET_TIMESPEC_NSECS_DOUBLE(&start_time)) / 1000000000.0;
	printf("--- %14.9f seconds.\n", total_time_1);

	printf("New algorithm: timing years from %04u to %04u ",
		first_year, last_year);
	previous_year = first_year;
	SDTIF_GetTimeNSecs(&start_time);
	for (count_1 = first_year; count_1 <= last_year; count_1++) {
		for (count_2 = first_month; count_2 <= last_month; count_2++) {
			...
			for (count_3 = first_day; count_3 <= max_dom; count_3++) {
				if ((count_1 == DATFUNCS_GREGORIAN_YEAR) &&
					...
					(count_3 <= DATFUNCS_GREGORIAN_LAST_DAY))
					continue;
				tmp_julian = ymdtojul(count_1, count_2, count_3);
				jultoymd(tmp_julian, &tmp_jdate.year, &tmp_jdate.month,
					&tmp_jdate.day);
			}
		}
	}
	SDTIF_GetTimeNSecs(&end_time);
	total_time_2 = (SDTIF_GET_TIMESPEC_NSECS_DOUBLE(&end_time) -
		SDTIF_GET_TIMESPEC_NSECS_DOUBLE(&start_time)) / 1000000000.0;
	printf("--- %14.9f seconds.\n", total_time_2);
	...
	fprintf(stderr, "%s\n",
		"--- --------- --- ----- ---- ---- -- ---- -- -------------- - --------------");
	fprintf(stderr,
		"Old algorithm for dates from %4d to %4d in %14.9f / %14.9f\n",
		first_year, last_year,
		total_time_1, total_time_1 / ((double) total_days));
	fprintf(stderr,
		"New algorithm for dates from %4d to %4d in %14.9f / %14.9f\n",
		first_year, last_year,
		total_time_2, total_time_2 / ((double) total_days));
	fprintf(stderr, "%s\n",
		"=== ========= === ===== ==== ==== == ==== == ============== = ==============");
...
/*	*********************************************************************** */
#ifndef NARGS
static int DoRegressionTest(int first_year, int last_year)
#else
static int DoRegressionTest(first_year, last_year)
int first_year;
int last_year;
#endif /* #ifndef NARGS */
{
...
"Date %04d-%02d-%02d --- julian = %11ld --- FAILED (returned %04d-%02d-%02d)\n",
		...
		if ((((count_1 / 100) * 100) == count_1) &&
			...
			printf("Dates %04u-01-01 through %04u-12-31 done.\n",
				previous_year, count_1);
