...
/*
	...
	File Description	:	Finds the highest record in a cache for a specified
								index method.
...
/* BOH
	...
	NAME			:	DCR_BasicGetHighest

	SYNOPSIS		:	void DCR_BasicGetHighest(cache_ptr, index_method,
	...
	DESCRIPTION	:	Gets the highest record in a cache using the specified index
						method.
	...
	PARAMETERS	:	Parameters to this function are as follow:
						...
						into which will be placed the number of the highest index
						element.

						(-) Because this function accesses the highest record using
						the specified index method, ``out_index_element`` will
						always be set to the number of cache records minus one.

						(-) ``out_index_element`` may be ''NULL'', in which case
						no action is taken.

						(.) ``out_record_element`` points to an unsigned integer
						into which will be placed the number of the highest record
						element.
   ...
   RETURNS     :  Void.
	...
	CAVEATS		:	This function assumes that the parameters passed to it are
						...
						function (``DCR_GetHighest``).

	SEE ALSO		:	DCR_GetHighest
						DCR_BasicGetLowest
	...
	OUTPUT INDEX:	DCR_BasicGetHighest
						DCR Cache Get Functions:DCR_BasicGetHighest
						DCR Cache Functions:DCR_BasicGetHighest

	PUBLISH XREF:	DCR_BasicGetHighest

	PUBLISH NAME:	DCR_BasicGetHighest

EOH */
/* *********************************************************************** */
void DCR_BasicGetHighest(cache_ptr, index_method, record_search_flag,
...
/* BOH
	...
	SYNOPSIS		:	return_code = DCR_GetHighest(cache_ptr, index_method,
   ...
   RETURNS     :  Returns from this function are as follow:
	...
	OUTPUT INDEX:	DCR_GetHighest
						DCR Cache Get Functions:DCR_GetHighest
						DCR Cache Functions:DCR_GetHighest

	PUBLISH XREF:	DCR_GetHighest

	PUBLISH NAME:	DCR_GetHighest

EOH */
/* *********************************************************************** */
int DCR_GetHighest(cache_ptr, index_method, record_search_flag,
	...
	*found_flag = DCR_RECORD_NOT_FOUND;
	...
	if ((return_code = DCR_ValidateRecSearchFlag(record_search_flag,
		...
		if ((return_code = DCR_MapDataAndIndex(cache_ptr, NULL, NULL,
			...
			DCR_BasicGetHighest(cache_ptr, index_method,
