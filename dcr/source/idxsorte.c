...
void DCR_IndexEmbedded(cache_ptr, index_method, entry_list)
DCR_CACHE    *cache_ptr;
...
void         *entry_list;
{
	...
	record_ptr                                         =
		((char *) cache_ptr->mmap_ptr) + ((unsigned int) record_offset);
	...
	DCR_SortEmbedded(cache_ptr, index_method, cache_ptr->record_count,
		entry_list);
...
void DCR_SortEmbedded(cache_ptr, index_method, entry_count, entry_list)
DCR_CACHE    *cache_ptr;
unsigned int  index_method;
...
void         *entry_list;
{
	...
	if ((cache_ptr->call_back_mask & DCR_CB_SORT_CACHE) &&
		...
		DCR_FALSE, index_method, 0, entry_count, entry_list, NULL, DCR_SUCCESS,
		NULL);
...
int main()
{
	...
	fprintf(stderr, "Test routine for DCR cache embedded indices\n");
	fprintf(stderr, "---- ------- --- --- ----- -------- -------\n\n");
