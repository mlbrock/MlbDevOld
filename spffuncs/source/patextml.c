...
/*
	...
	File Description	:	Parses text with line continuation.
...
/*	***********************************************************************	*/
#ifndef NARGS
int SPF_PARSE_TextMLList(const SPF_SPEC *spec_ptr, const SPF_PARSE *parse_ptr,
	unsigned int *text_count, char ***text_list, char *error_text)
#else
int SPF_PARSE_TextMLList(spec_ptr, parse_ptr, text_count, text_list,
	error_text)
...
#endif /* #ifndef NARGS */
{
	...
	if ((return_code = SPF_PARSE_TextML(spec_ptr, parse_ptr, &text_ptr,
...
/*	***********************************************************************	*/
#ifndef NARGS
int SPF_PARSE_TextML(const SPF_SPEC *spec_ptr, const SPF_PARSE *parse_ptr,
	char **text_ptr, char *error_text)
#else
int SPF_PARSE_TextML(spec_ptr, parse_ptr, text_ptr, error_text)
const SPF_SPEC   *spec_ptr;
const SPF_PARSE  *parse_ptr;
char            **text_ptr;
char             *error_text;
#endif /* #ifndef NARGS */
{
	...
	*text_ptr = NULL;
	...
	if ((return_code = SPF_GetSubParse(parse_ptr, &tmp_parse, error_text)) ==
		SPF_SUCCESS) {
		if ((return_code = SPF_FixMultiLineString(&tmp_parse, error_text)) !=
			SPF_SUCCESS)
			;
		else if ((tmp_parse.length < spec_ptr->min_length) ||
