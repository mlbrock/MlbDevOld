...
#define RUNPV_FILE_NAME_CFG				"RunPV.cfg"
#define RUNPV_FILE_NAME_LOG				"RunPV.log"

static int  GetFileArgs(char **system_cmd, char *error_text);
static void LogResults(int error_code, int system_code, const char *text);
...
int main()
{
	...
	if ((return_code = GetFileArgs(&system_cmd, error_text)) ==
		EXIT_SUCCESS) {
		return_code = system(system_cmd);
		LogResults(0, return_code, system_cmd);
		free(system_cmd);
	}
	else
		LogResults(return_code, 0, error_text);
	...
	char **file_name_log, char *error_text)
{
	...
	*file_name_log = NULL;
	...
	if ((return_code = AllocFileName(file_name_cfg, argv_0, len_0,
		...
		return_code = AllocFileName(file_name_log, argv_0, len_0, "log", "log",
			error_text);
...
static int GetFileArgs(char **system_cmd, char *error_text)
{
	...
	*system_cmd = NULL;
	...
	if ((file_ptr = fopen(RUNPV_FILE_NAME_CFG, "rb")) != NULL) {
		...
		if ((*system_cmd = ((char *) calloc(buffer_length, sizeof(char)))) !=
			...
			sprintf(error_text, "%s %u bytes for the file '%-.500s'.",
				"Unable to allocate", buffer_length, RUNPV_FILE_NAME_CFG);
			return_code = EXIT_FAILURE;
		}
		fclose(file_ptr);
	}
		...
		sprintf(error_text, "%s '%-.500s' for reading.",
			"Unable to open the file", RUNPV_FILE_NAME_CFG);
		return_code = EXIT_FAILURE;
	}
...
# endif /* #if defined(__EXTENSIONS__ || _REENTRANT || _POSIX_C_SOURCE) */
	...
	if ((file_ptr = fopen(RUNPV_FILE_NAME_LOG, "a")) != NULL) {
