...
/*
	...
	File Description	:	Locks a recursive mutex.
...
/*	***********************************************************************	*/
COMPAT_FN_DECL_STATIC(int MTIF_MTR_LockFacility,
	(MTIF_MTR_HANDLE *mtr_handle_ptr, char *error_text));
/* *********************************************************************** */

/* *********************************************************************** */
#ifndef NARGS
int MTIF_MTR_Lock(MTIF_MTR_HANDLE *mtr_handle_ptr, char *error_text)
#else
int MTIF_MTR_Lock(mtr_handle_ptr, error_text)
		...
		if (mtr_handle_ptr->lock_count < ULONG_MAX)
			...
			if (error_text != NULL)
				strcat(val2str_ulong_cat(MTIF_THR_Self(), 10, "",
					...
					"'MTIF_MTR_Lock()' on thread identifier ")),
					": maximum recusive lock count reached.");
			return_code = MTIF_OP_FAILURE_MTR;
		}
	}
	else
		return_code = MTIF_MTR_LockFacility(mtr_handle_ptr,
			(error_text != NULL) ? tmp_error_text : NULL);

	if ((return_code != MTIF_SUCCESS) && (error_text != NULL))
		strcat(strcpy(error_text,
			"Attempt to perform 'MTIF_MTR_Lock()' failed: "), tmp_error_text);
...
/*
	ADF NOTE: Implemented in order to provide an orthangonal interface.
*/
/* *********************************************************************** */
#ifndef NARGS
int MTIF_MTR_LockReader(MTIF_MTR_HANDLE *mtr_handle_ptr, char *error_text)
#else
int MTIF_MTR_LockReader(mtr_handle_ptr, error_text)
MTIF_MTR_HANDLE *mtr_handle_ptr;
char            *error_text;
#endif /* #ifndef NARGS */
{
	return(MTIF_MTR_Lock(mtr_handle_ptr, error_text));
}
/* *********************************************************************** */
...
/* *********************************************************************** */
#ifndef NARGS
int MTIF_MTR_LockWriter(MTIF_MTR_HANDLE *mtr_handle_ptr, char *error_text)
#else
int MTIF_MTR_LockWriter(mtr_handle_ptr, error_text)
MTIF_MTR_HANDLE *mtr_handle_ptr;
char            *error_text;
#endif /* #ifndef NARGS */
{
...
/* *********************************************************************** */
#ifndef NARGS
static int MTIF_MTR_LockFacility(MTIF_MTR_HANDLE *mtr_handle_ptr,
	char *error_text)
#else
static int MTIF_MTR_LockFacility(mtr_handle_ptr, error_text)
MTIF_MTR_HANDLE *mtr_handle_ptr;
char            *error_text;
#endif /* #ifndef NARGS */
{
	...
	if (mtr_handle_ptr->static_flag == MTIF_TRUE) {
		...
		if ((return_code = MTIF_SUPP_WIN32_CreateMutex(MTIF_TRUE, mtr_name,
			...
			if (GetLastError() == ERROR_ALREADY_EXISTS) {
				...
				while ((mtr_handle_ptr->static_flag == MTIF_TRUE) ||
					...
					MTIF_THR_Yield();
				return_code = MTIF_MTR_Lock(mtr_handle_ptr, error_text);
			}
			else {
		...
		mtr_handle_ptr->mtr_data, INFINITE, error_text)) == MTIF_SUCCESS) {
		mtr_handle_ptr->process_id = MTIF_THR_SelfProcess();
		mtr_handle_ptr->thread_id  = MTIF_THR_Self();
		mtr_handle_ptr->lock_count = 1L;
		return_code                = MTIF_SUCCESS;
...
#endif /* #ifndef NARGS */
{
	...
	if (!(return_code = mutex_lock(&mtr_handle_ptr->mtr_data))) {
		mtr_handle_ptr->static_flag = MTIF_FALSE;
		mtr_handle_ptr->process_id  = MTIF_THR_SelfProcess();
		mtr_handle_ptr->thread_id   = MTIF_THR_Self();
		mtr_handle_ptr->lock_count  = 1L;
...
#endif /* #ifndef NARGS */
{
	...
	if (!(return_code = pthread_mutex_lock(&mtr_handle_ptr->mtr_data))) {
		mtr_handle_ptr->static_flag = MTIF_FALSE;
		mtr_handle_ptr->process_id  = MTIF_THR_SelfProcess();
		mtr_handle_ptr->thread_id   = MTIF_THR_Self();
		mtr_handle_ptr->lock_count  = 1L;
...
#endif /* #ifndef NARGS */
{
	...
	if (error_text != NULL)
		strcpy(error_text, "Function 'MTIF_MTR_Lock()' not supported.");
