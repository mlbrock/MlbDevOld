...
/*
	...
	File Description	:	Tries to lock a basic mutex.
...
/*	***********************************************************************	*/
COMPAT_FN_DECL_STATIC(int MTIF_MTX_TryLockFacility,
	(MTIF_MTX_HANDLE *mtx_handle_ptr, char *error_text));
/* *********************************************************************** */

/* *********************************************************************** */
#ifndef NARGS
int MTIF_MTX_TryLock(MTIF_MTX_HANDLE *mtx_handle_ptr, char *error_text)
#else
int MTIF_MTX_TryLock(mtx_handle_ptr, error_text)
	...
	if ((return_code = MTIF_MTX_Check(mtx_handle_ptr,
		...
		if (error_text != NULL)
			chrcat(val2str_ulong_cat(MTIF_THR_Self(), 10, "",
				...
				"'MTIF_MTX_TryLock()' on thread identifier ")), '.');
		return_code = MTIF_OP_FAILURE_MTX;
	}
	else
		return_code = MTIF_MTX_TryLockFacility(mtx_handle_ptr,
			(error_text != NULL) ? tmp_error_text : NULL);

	if ((return_code != MTIF_SUCCESS) && (error_text != NULL))
		strcat(strcpy(error_text,
			"Attempt to perform 'MTIF_MTX_TryLock()' failed: "), tmp_error_text);
...
/* *********************************************************************** */
#ifndef NARGS
int MTIF_MTX_TryLockReader(MTIF_MTX_HANDLE *mtx_handle_ptr, char *error_text)
#else
int MTIF_MTX_TryLockReader(mtx_handle_ptr, error_text)
MTIF_MTX_HANDLE *mtx_handle_ptr;
char            *error_text;
#endif /* #ifndef NARGS */
{
	return(MTIF_MTX_TryLock(mtx_handle_ptr, error_text));
}
/* *********************************************************************** */
...
/* *********************************************************************** */
#ifndef NARGS
int MTIF_MTX_TryLockWriter(MTIF_MTX_HANDLE *mtx_handle_ptr, char *error_text)
#else
int MTIF_MTX_TryLockWriter(mtx_handle_ptr, error_text)
MTIF_MTX_HANDLE *mtx_handle_ptr;
char            *error_text;
#endif /* #ifndef NARGS */
{
...
/* *********************************************************************** */
#ifndef NARGS
static int MTIF_MTX_TryLockFacility(MTIF_MTX_HANDLE *mtx_handle_ptr,
	char *error_text)
#else
static int MTIF_MTX_TryLockFacility(mtx_handle_ptr, error_text)
MTIF_MTX_HANDLE *mtx_handle_ptr;
char            *error_text;
#endif /* #ifndef NARGS */
{
	...
	if (mtx_handle_ptr->static_flag == MTIF_TRUE) {
		...
		if ((return_code = MTIF_SUPP_WIN32_CreateMutex(MTIF_TRUE, mtx_name,
			...
			if (GetLastError() == ERROR_ALREADY_EXISTS) {
				...
				while ((mtx_handle_ptr->static_flag == MTIF_TRUE) ||
					...
					MTIF_THR_Yield();
				return_code = MTIF_MTX_TryLock(mtx_handle_ptr, error_text);
			}
			else {
		...
		mtx_handle_ptr->mtx_data, 0, error_text)) == MTIF_SUCCESS) {
		mtx_handle_ptr->process_id = MTIF_THR_SelfProcess();
		mtx_handle_ptr->thread_id  = MTIF_THR_Self();
		return_code                = MTIF_SUCCESS;
	}
...
#endif /* #ifndef NARGS */
{
	...
	if (!(return_code = mutex_trylock(&mtx_handle_ptr->mtx_data))) {
		mtx_handle_ptr->static_flag = MTIF_FALSE;
		mtx_handle_ptr->process_id  = MTIF_THR_SelfProcess();
		mtx_handle_ptr->thread_id   = MTIF_THR_Self();
		return_code                 = MTIF_SUCCESS;
...
#endif /* #ifndef NARGS */
{
	...
	if (!(return_code = pthread_mutex_trylock(&mtx_handle_ptr->mtx_data))) {
		mtx_handle_ptr->static_flag = MTIF_FALSE;
		mtx_handle_ptr->process_id  = MTIF_THR_SelfProcess();
		mtx_handle_ptr->thread_id   = MTIF_THR_Self();
		return_code                 = MTIF_SUCCESS;
...
#endif /* #ifndef NARGS */
{
	...
	if (error_text != NULL)
		strcpy(error_text, "Function 'MTIF_MTX_TryLock()' not supported.");
