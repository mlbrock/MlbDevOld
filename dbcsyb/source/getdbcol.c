...
/*	***********************************************************************	*/
/*	***********************************************************************	*/
COMPAT_EXTERN_DATA_DEF const MDDL   DBC_SYB_GetDBColMemberList[] = {
	{
		"type",
		STR_sizeof(DBC_SYB_DB_COL, type),
		STR_sizeof(DBC_SYB_DB_COL, type),
		offsetof(DBC_SYB_DB_COL, type),
		1,
		"C",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	{
		"table_type",
		STR_sizeof(DBC_SYB_DB_COL, table_type),
		STR_sizeof(DBC_SYB_DB_COL, table_type),
		offsetof(DBC_SYB_DB_COL, table_type),
		1,
		"C",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	{
		"user_type",
		STR_sizeof(DBC_SYB_DB_COL, user_type),
		STR_sizeof(DBC_SYB_DB_COL, user_type),
		offsetof(DBC_SYB_DB_COL, user_type),
		1,
		"s",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	},
	{
		...
		STR_sizeof(DBC_SYB_DB_COL, name),
		offsetof(DBC_SYB_DB_COL, name),
		STR_sizeof(DBC_SYB_DB_COL, name) - 1,
		"c",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	{
		"length",
		STR_sizeof(DBC_SYB_DB_COL, length),
		STR_sizeof(DBC_SYB_DB_COL, length),
		offsetof(DBC_SYB_DB_COL, length),
		1,
		"C",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	},
	{
		...
		STR_sizeof(DBC_SYB_DB_COL, type_name),
		offsetof(DBC_SYB_DB_COL, type_name),
		STR_sizeof(DBC_SYB_DB_COL, type_name) - 1,
		"c",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	{
		"offset",
		STR_sizeof(DBC_SYB_DB_COL, offset),
		STR_sizeof(DBC_SYB_DB_COL, offset),
		offsetof(DBC_SYB_DB_COL, offset),
		1,
		"s",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	},
	{
		...
		STR_sizeof(DBC_SYB_DB_COL, default_name),
		offsetof(DBC_SYB_DB_COL, default_name),
		STR_sizeof(DBC_SYB_DB_COL, default_name) - 1,
		"c",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	},
	{
		...
		STR_sizeof(DBC_SYB_DB_COL, rule_name),
		offsetof(DBC_SYB_DB_COL, rule_name),
		STR_sizeof(DBC_SYB_DB_COL, rule_name) - 1,
		"c",
		0,
		MDDL_DTA_EMPTY_LIST,
	...
	{
		"status",
		STR_sizeof(DBC_SYB_DB_COL, status),
		STR_sizeof(DBC_SYB_DB_COL, status),
		offsetof(DBC_SYB_DB_COL, status),
		1,
		"C",
		0,
		MDDL_DTA_EMPTY_LIST,
...
COMPAT_EXTERN_DATA_DEF const unsigned int DBC_SYB_GetDBColMemberCount  =
	sizeof(DBC_SYB_GetDBColMemberList) /
	sizeof(DBC_SYB_GetDBColMemberList[0]);
/*	***********************************************************************	*/
...
/* *********************************************************************** */
#ifndef NARGS
int DBC_SYB_GetDBColumn(const char *spec_string, const char *in_table_name,
	unsigned int *column_count, DBC_SYB_DB_COL **column_list, char *error_text)
#else
int DBC_SYB_GetDBColumn(spec_string, in_table_name, column_count, column_list,
	error_text)
const char      *spec_string;
const char      *in_table_name;
unsigned int    *column_count;
DBC_SYB_DB_COL **column_list;
char            *error_text;
#endif /* #ifndef NARGS */
{
	int         return_code;
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetDBCols",
		...
		return_code = DBC_SYB_GetDBColumnBasic(dbc_context_data.dbc_control_ptr,
...
/* *********************************************************************** */
#ifndef NARGS
int DBC_SYB_GetDBColumnBasic(void *dbc_control_ptr, const char *in_table_name,
	unsigned int *column_count, DBC_SYB_DB_COL **column_list, char *error_text)
#else
int DBC_SYB_GetDBColumnBasic(dbc_control_ptr, in_table_name, column_count,
...
#endif /* #ifndef NARGS */
{
	...
	*column_list  = NULL;
	...
	if ((return_code = (*dbc_funcs.get_rows)(dbc_control_ptr,
		DBC_SYB_GetDBColMemberCount, DBC_SYB_GetDBColMemberList, sql_command,
		...
		if (!(*column_count)) {
			...
			for (count_1 = 0; count_1 < *column_count; count_1++)
				(*column_list)[count_1].basic_type =
					((*column_list)[count_1].type == SYBVARCHAR)   ?
					SYBCHAR     :
					((*column_list)[count_1].type == SYBVARBINARY) ?
					SYBBINARY   :
					((*column_list)[count_1].type == SYBDATETIMN)  ?
					SYBDATETIME :
					((*column_list)[count_1].type == SYBMONEYN)    ?
					SYBMONEY    :
					((*column_list)[count_1].type == SYBFLTN)      ?
					SYBFLT8     :
					(((*column_list)[count_1].type == SYBINTN) &&
					...
					SYBINT4     : (*column_list)[count_1].type;
		}
	}
...
#endif /* #ifndef NARGS */
{
	...
	DBC_SYB_DB_COL *column_list  = NULL;
	unsigned int    count_1;
	char            error_text[DBC_MAX_ERROR_TEXT];

	STR_EMIT_CharLine('=', 78, NULL, NULL);

	fprintf(stderr, "Test routine for 'DBC_SYB_GetDBColumn()'\n");
	fprintf(stderr, "---- ------- --- -----------------------\n");
	...
	for (count_1 = 2; count_1 < ((unsigned int) argc); count_1++) {
		...
		if ((return_code = DBC_SYB_GetDBColumn(argv[1], argv[count_1],
			&column_count, &column_list, error_text)) != DBC_SUCCESS)
			break;
		fprintf(stderr, "%u columns in table.\n", column_count);
		DBC_SYB_EMIT_DBColumnListIdx(column_count, column_list, NULL, NULL);
