...
/* *********************************************************************** */
#ifndef NARGS
int DBC_SYB_GetDB(const char *spec_string, unsigned int *database_count,
	DBC_SYB_DB **database_list, char *error_text)
#else
int DBC_SYB_GetDB(spec_string, database_count, database_list, error_text)
const char    *spec_string;
unsigned int  *database_count;
DBC_SYB_DB   **database_list;
char          *error_text;
#endif /* #ifndef NARGS */
{
	int         return_code;
	...
	if ((return_code = DBC_Open(&dbc_context_data, NULL, "DBC:GetDBs",
		...
		return_code = DBC_SYB_GetDBBasic(dbc_context_data.dbc_control_ptr,
...
/* *********************************************************************** */
#ifndef NARGS
int DBC_SYB_GetDBBasic(void *dbc_control_ptr, unsigned int *database_count,
	DBC_SYB_DB **database_list, char *error_text)
#else
int DBC_SYB_GetDBBasic(dbc_control_ptr, database_count, database_list,
	error_text)
...
#endif /* #ifndef NARGS */
{
	...
	DBC_SYB_DB_DB *db_database_list  = NULL;
	unsigned int   count_1;

	*database_count = 0;
	*database_list  = NULL;

	if ((return_code = DBC_SYB_GetDBDBBasic(dbc_control_ptr,
		&db_database_count, &db_database_list, error_text)) == DBC_SUCCESS) {
		if ((*database_list = ((DBC_SYB_DB *) calloc(db_database_count,
			sizeof(DBC_SYB_DB)))) == NULL) {
			STR_AllocMsgList(db_database_count, sizeof(DBC_SYB_DB),
...
#endif /* #ifndef NARGS */
{
	...
	DBC_SYB_DB   *database_list  = NULL;
	unsigned int  count_1;
	char          error_text[DBC_MAX_ERROR_TEXT];

	STR_EMIT_CharLine('=', 78, NULL, NULL);

	fprintf(stderr, "Test routine for 'DBC_SYB_GetDB()'\n");
	fprintf(stderr, "---- ------- --- -----------------\n");
	...
	if ((return_code = DBC_SYB_GetDB(argv[1], &database_count,
		&database_list, error_text)) == DBC_SUCCESS) {
		fprintf(stderr, "%u databases found.\n", database_count);
		DBC_SYB_EMIT_DBListIdx(database_count, database_list, NULL, NULL);
