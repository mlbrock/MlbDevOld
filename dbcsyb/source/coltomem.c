	...
	if (!column_count) {
		...
		*member_count = column_count;
		for (count_1 = 0; count_1 < column_count; count_1++) {
			...
			(*member_list)[count_1].length_function = DBC_SYB_SetDisplayLength;
			(*member_list)[count_1].dump_function   = DBC_SYB_DumpField;
