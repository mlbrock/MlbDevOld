...
EOH */
/*	***********************************************************************	*/
int MMAP_Sync(address, length, sync_flags, error_text)
void         *address;
unsigned int  length;
...
char         *error_text;
{
	...
	if (((return_code =
		...
		MMAP_CheckLength(length, tmp_error_text))        == MMAP_SUCCESS) &&
		 ((return_code =
		MMAP_CheckSyncFlags(sync_flags, tmp_error_text)) == MMAP_SUCCESS)) {
		if (MMAP_msync(address, length, sync_flags))
	...
	if ((return_code != MMAP_SUCCESS) && (error_text != NULL))
		sprintf(error_text, "MMAP_Sync(%08lX, %u, %u, %08lX) failed: %s",
			((unsigned long) address), length, sync_flags,
