...
/*	BOH
   ...
   DESCRIPTION :	Populates a ''DBC_FUNCS'' structure with pointers to the
						functions which implement the DBC Sybase DB-LIB facility.
   ...
   NOTES       :	If the DBC Sybase DB-LIB facility library ``dbcdblib.a`` has
   ...
   OUTPUT INDEX:	DBC_DBLIB_AttachFacility
						DBC Sybase DB-LIB Functions:DBC_DBLIB_AttachFacility
...
/*	***********************************************************************	*/
#ifndef NARGS
void DBC_DBLIB_AttachFacility(DBC_FUNCS *funcs_ptr)
#else
void DBC_DBLIB_AttachFacility(funcs_ptr)
DBC_FUNCS *funcs_ptr;
#endif /* #ifndef NARGS */
{
	if (funcs_ptr != NULL) {
		...
		funcs_ptr->bind               =
			...
			char *error_text), DBC_DBLIB_BindMembers);
		funcs_ptr->cancel             =
		...
		funcs_ptr->cancel_query       =
			COMPAT_FN_CAST(int, (void *dbc_control_ptr, char *error_text),
			DBC_DBLIB_CancelQuery);
		funcs_ptr->close              =
			COMPAT_FN_CAST(void, (void **dbc_control_ptr), DBC_DBLIB_Close);
		funcs_ptr->do_sql             =
			COMPAT_FN_CAST(int, (void *dbc_control_ptr, const char *sql_command,
			char *error_text), DBC_DBLIB_DoSQLCommand);
		funcs_ptr->get_id             = DBC_DBLIB_GetID;
		funcs_ptr->get_rows           =
			COMPAT_FN_CAST(int, (void *dbc_control_ptr, unsigned int member_count,
			...
			char *error_text), DBC_DBLIB_GetRows);
		funcs_ptr->get_rows_basic     =
			COMPAT_FN_CAST(int, (void *dbc_control_ptr,
			const DBC_GETROWS *get_rows_ptr, unsigned int *out_row_count,
			void **out_row_list, char *error_text), DBC_DBLIB_GetRowsBasic);
		funcs_ptr->get_vendor_name    = DBC_DBLIB_GetVendorName;
		funcs_ptr->get_version        = DBC_DBLIB_GetVersion;
		funcs_ptr->get_version_string = DBC_DBLIB_GetVersionString;
		funcs_ptr->init               = DBC_DBLIB_Initialize;
		funcs_ptr->open               =
			COMPAT_FN_CAST(int, (void *dbc_control_ptr, const char *conn_type,
			const char *conn_string, MLOG *mlog_ptr, DBC_FLAG flags,
			void *user_data_ptr, char *error_text), DBC_DBLIB_Open);
