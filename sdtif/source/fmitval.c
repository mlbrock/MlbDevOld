...
/*
	...
	File Description	:	Formats 'struct timeval' time measurement values as
...
/* BOH
	...
	NAME        :  SDTIF_FormatInterval_timeval

	SYNOPSIS    :  string_ptr = SDTIF_FormatInterval_timeval(in_time, buffer);
	...
	CAVEATS     :  The area to which the ``buffer`` parameter points
						should be at least ''SDTIF_FMTI_TIME_LEN_TIMEVAL + 1''
						(26 + 1) bytes in size.
	...
	OUTPUT INDEX:	SDTIF_FormatInterval_timeval
						Date/Time Functions:SDTIF_FormatInterval_timeval
						SDTIF:Functions:SDTIF_FormatInterval_timeval
						SDTIF:Date/Time Functions:SDTIF_FormatInterval_timeval
						System Date/Time Interface Facility Functions:See SDTIF

	PUBLISH XREF:	SDTIF_FormatInterval_timeval

	PUBLISH NAME:	SDTIF_FormatInterval_timeval
...
/*	***********************************************************************	*/
#ifndef NARGS
char *SDTIF_FormatInterval_timeval(const struct timeval *in_time, char *buffer)
#else
char *SDTIF_FormatInterval_timeval(in_time, buffer)
const struct timeval *in_time;
char                 *buffer;
#endif /* #ifndef NARGS */
{
